...
var r = class _r {
  constructor(t, i2, s2 = 0, n2 = t ? t.byteLength : 0) {
    this._tag = 0, this._dataType = e2.unknown, this._init(t, i2, s2, n2);
  }
  _init(t, e3, i2, s2) {
  ...
  clone() {
    return new _r(this._data, this._dataView, this._pos, this._end);
  }
  pos() {
    return this._pos;
  }
  ...
  processMessageWithArgs(t, e3, i2, s2) {
    const n2 = this.getMessage(), r2 = t(n2, e3, i2, s2);
    return n2.release(), r2;
  }
  processMessage(t) {
  ...
  _decodeSVarint() {
    const t = this._data;
    let e3, i2 = 0, s2 = 0;
    const n2 = 1 & t[this._pos];
    if (s2 = t[this._pos++], i2 |= 127 & s2, 0 == (128 & s2))
      return n2 ? -(i2 + 1) / 2 : i2 / 2;
    if (s2 = t[this._pos++], i2 |= (127 & s2) << 7, 0 == (128 & s2))
      return n2 ? -(i2 + 1) / 2 : i2 / 2;
    if (s2 = t[this._pos++], i2 |= (127 & s2) << 14, 0 == (128 & s2))
      return n2 ? -(i2 + 1) / 2 : i2 / 2;
    if (s2 = t[this._pos++], i2 |= (127 & s2) << 21, 0 == (128 & s2))
      return n2 ? -(i2 + 1) / 2 : i2 / 2;
    if (s2 = t[this._pos++], i2 += 268435456 * (127 & s2), 0 == (128 & s2))
      return n2 ? -(i2 + 1) / 2 : i2 / 2;
    if (s2 = t[this._pos++], i2 += 34359738368 * (127 & s2), 0 == (128 & s2))
      return n2 ? -(i2 + 1) / 2 : i2 / 2;
    if (s2 = t[this._pos++], i2 += 4398046511104 * (127 & s2), 0 == (128 & s2))
      return n2 ? -(i2 + 1) / 2 : i2 / 2;
    if (e3 = BigInt(i2), s2 = t[this._pos++], e3 += 0x2000000000000n * BigInt(127 & s2), 0 == (128 & s2))
      return Number(n2 ? -(e3 + 1n) / 2n : e3 / 2n);
    if (s2 = t[this._pos++], e3 += 0x100000000000000n * BigInt(127 & s2), 0 == (128 & s2))
      return Number(n2 ? -(e3 + 1n) / 2n : e3 / 2n);
    if (s2 = t[this._pos++], e3 += 0x8000000000000000n * BigInt(127 & s2), 0 == (128 & s2))
      return Number(n2 ? -(e3 + 1n) / 2n : e3 / 2n);
    throw new Error("Varint too long!");
  }
  _getLength() {
  ...
  _toString(t, e3, i2) {
    if ((i2 = Math.min(this._end, i2)) - e3 > n) {
      const n2 = t.subarray(e3, i2);
      return s.decode(n2);
    }
    let r2 = "", h = "";
    for (let s2 = e3; s2 < i2; ++s2) {
      const e4 = t[s2];
      128 & e4 ? h += "%" + e4.toString(16) : (r2 += decodeURIComponent(h) + String.fromCharCode(e4), h = "");
    }
    return h.length && (r2 += decodeURIComponent(h)), r2;
  }
  _toBytes(t, e3, i2) {
    return i2 = Math.min(this._end, i2), new Uint8Array(t.buffer, e3, i2 - e3);
  }
};
r.pool = new e(r, void 0, (t) => {
...
//# sourceMappingURL=chunk-OYPRFNII.js.map
