import {
  o
} from "./chunk-EUTNDAIV.js";
import {
  a
} from "./chunk-SJZIFARA.js";
import {
  r
} from "./chunk-WJ3HJKNC.js";
import {
  has
} from "./chunk-4V6374PV.js";

// node_modules/@arcgis/core/chunks/pe.js
var t;
var o2 = null;
function r2() {
  return !!o2;
}
function n() {
  return !!has("esri-wasm");
}
function _() {
  return t || (t = import("./pe-wasm-ZA54EZSW.js").then((e2) => e2.p).then(({ default: t3 }) => t3({ locateFile: (t4) => a(`esri/geometry/support/${t4}`) })).then((e2) => {
    l(e2);
  }), t);
}
var P;
var s;
var E;
!function(e2) {
  function t3(e3, t4, r5) {
    o2.ensureCache.prepare();
    const n3 = M(r5), _3 = r5 === n3, P3 = o2.ensureFloat64(n3), s4 = o2._pe_geog_to_proj(o2.getPointer(e3), t4, P3);
    return s4 && A(r5, t4, P3, _3), s4;
  }
  function r4(e3, o5, r5, _3) {
    switch (_3) {
      case s.PE_TRANSFORM_P_TO_G:
        return n2(e3, o5, r5);
      case s.PE_TRANSFORM_G_TO_P:
        return t3(e3, o5, r5);
    }
    return 0;
  }
  function n2(e3, t4, o5) {
    return _2(e3, t4, o5, 0);
  }
  function _2(e3, t4, r5, n3) {
    o2.ensureCache.prepare();
    const _3 = M(r5), P3 = r5 === _3, s4 = o2.ensureFloat64(_3), E4 = o2._pe_proj_to_geog_center(o2.getPointer(e3), t4, s4, n3);
    return E4 && A(r5, t4, s4, P3), E4;
  }
  e2.geogToProj = t3, e2.projGeog = r4, e2.projToGeog = n2, e2.projToGeogCenter = _2;
}(P || (P = {})), function(e2) {
  function t3() {
    e2.PE_BUFFER_MAX = o2.PeDefs.prototype.PE_BUFFER_MAX, e2.PE_NAME_MAX = o2.PeDefs.prototype.PE_NAME_MAX, e2.PE_MGRS_MAX = o2.PeDefs.prototype.PE_MGRS_MAX, e2.PE_USNG_MAX = o2.PeDefs.prototype.PE_USNG_MAX, e2.PE_DD_MAX = o2.PeDefs.prototype.PE_DD_MAX, e2.PE_DDM_MAX = o2.PeDefs.prototype.PE_DDM_MAX, e2.PE_DMS_MAX = o2.PeDefs.prototype.PE_DMS_MAX, e2.PE_UTM_MAX = o2.PeDefs.prototype.PE_UTM_MAX, e2.PE_PARM_MAX = o2.PeDefs.prototype.PE_PARM_MAX, e2.PE_TYPE_NONE = o2.PeDefs.prototype.PE_TYPE_NONE, e2.PE_TYPE_GEOGCS = o2.PeDefs.prototype.PE_TYPE_GEOGCS, e2.PE_TYPE_PROJCS = o2.PeDefs.prototype.PE_TYPE_PROJCS, e2.PE_TYPE_GEOGTRAN = o2.PeDefs.prototype.PE_TYPE_GEOGTRAN, e2.PE_TYPE_COORDSYS = o2.PeDefs.prototype.PE_TYPE_COORDSYS, e2.PE_TYPE_UNIT = o2.PeDefs.prototype.PE_TYPE_UNIT, e2.PE_TYPE_LINUNIT = o2.PeDefs.prototype.PE_TYPE_LINUNIT, e2.PE_STR_OPTS_NONE = o2.PeDefs.prototype.PE_STR_OPTS_NONE, e2.PE_STR_AUTH_NONE = o2.PeDefs.prototype.PE_STR_AUTH_NONE, e2.PE_STR_AUTH_TOP = o2.PeDefs.prototype.PE_STR_AUTH_TOP, e2.PE_STR_NAME_CANON = o2.PeDefs.prototype.PE_STR_NAME_CANON, e2.PE_STR_FMT_WKT = o2.PeDefs.prototype.PE_STR_FMT_WKT, e2.PE_STR_FMT_WKT2 = o2.PeDefs.prototype.PE_STR_FMT_WKT2, e2.PE_PARM_X0 = o2.PeDefs.prototype.PE_PARM_X0, e2.PE_PARM_ND = o2.PeDefs.prototype.PE_PARM_ND, e2.PE_TRANSFORM_1_TO_2 = o2.PeDefs.prototype.PE_TRANSFORM_1_TO_2, e2.PE_TRANSFORM_2_TO_1 = o2.PeDefs.prototype.PE_TRANSFORM_2_TO_1, e2.PE_TRANSFORM_P_TO_G = o2.PeDefs.prototype.PE_TRANSFORM_P_TO_G, e2.PE_TRANSFORM_G_TO_P = o2.PeDefs.prototype.PE_TRANSFORM_G_TO_P, e2.PE_HORIZON_RECT = o2.PeDefs.prototype.PE_HORIZON_RECT, e2.PE_HORIZON_POLY = o2.PeDefs.prototype.PE_HORIZON_POLY, e2.PE_HORIZON_LINE = o2.PeDefs.prototype.PE_HORIZON_LINE, e2.PE_HORIZON_DELTA = o2.PeDefs.prototype.PE_HORIZON_DELTA;
  }
  e2.init = t3;
}(s || (s = {})), function(e2) {
  const t3 = {}, r4 = {}, n2 = (e3) => {
    if (e3) {
      const t4 = e3.getType();
      switch (t4) {
        case s.PE_TYPE_GEOGCS:
          e3 = o2.castObject(e3, o2.PeGeogcs);
          break;
        case s.PE_TYPE_PROJCS:
          e3 = o2.castObject(e3, o2.PeProjcs);
          break;
        case s.PE_TYPE_GEOGTRAN:
          e3 = o2.castObject(e3, o2.PeGeogtran);
          break;
        default:
          t4 & s.PE_TYPE_UNIT && (e3 = o2.castObject(e3, o2.PeUnit));
      }
    }
    return e3;
  };
  function _2() {
    o2.PeFactory.prototype.initialize(null);
  }
  function P3(e3) {
    return E4(s.PE_TYPE_COORDSYS, e3);
  }
  function E4(e3, r5) {
    let _3 = null, P4 = t3[e3];
    if (P4 || (P4 = {}, t3[e3] = P4), P4.hasOwnProperty(String(r5)))
      _3 = P4[r5];
    else {
      const t4 = o2.PeFactory.prototype.factoryByType(e3, r5);
      o2.compare(t4, o2.NULL) || (_3 = t4, P4[r5] = _3);
    }
    return _3 = n2(_3), _3;
  }
  function i3(e3, t4) {
    let _3 = null, P4 = r4[e3];
    if (P4 || (P4 = {}, r4[e3] = P4), P4.hasOwnProperty(t4))
      _3 = P4[t4];
    else {
      const r5 = o2.PeFactory.prototype.fromString(e3, t4);
      o2.compare(r5, o2.NULL) || (_3 = r5, P4[t4] = _3);
    }
    return _3 = n2(_3), _3;
  }
  function p4(e3) {
    return E4(s.PE_TYPE_GEOGCS, e3);
  }
  function u4(e3) {
    return E4(s.PE_TYPE_GEOGTRAN, e3);
  }
  function a3(e3) {
    return o2.PeFactory.prototype.getCode(e3);
  }
  function c3(e3) {
    return E4(s.PE_TYPE_PROJCS, e3);
  }
  function T4(e3) {
    return E4(s.PE_TYPE_UNIT, e3);
  }
  e2.initialize = _2, e2.coordsys = P3, e2.factoryByType = E4, e2.fromString = i3, e2.geogcs = p4, e2.geogtran = u4, e2.getCode = a3, e2.projcs = c3, e2.unit = T4;
}(E || (E = {}));
var i = null;
var p;
var u;
var a2;
var c;
var T;
var g;
var f;
var S;
var O;
function l(e2) {
  function t3(e3, t4, o5) {
    e3[t4] = o5(e3[t4]);
  }
  o2 = e2, s.init(), p.init(), T.init(), f.init(), S.init(), i = class extends o2.PeGCSExtent {
    destroy() {
      o2.destroy(this);
    }
  };
  const r4 = [o2.PeDatum, o2.PeGeogcs, o2.PeGeogtran, o2.PeObject, o2.PeParameter, o2.PePrimem, o2.PeProjcs, o2.PeSpheroid, o2.PeUnit];
  for (const o5 of r4)
    t3(o5.prototype, "getName", (e3) => function() {
      return e3.call(this, new Array(s.PE_NAME_MAX));
    });
  for (const P3 of [o2.PeGeogtran, o2.PeProjcs])
    t3(P3.prototype, "getParameters", (e3) => function() {
      const t4 = new Array(s.PE_PARM_MAX);
      let r5 = e3.call(this);
      for (let e4 = 0; e4 < t4.length; e4++) {
        const n3 = o2.getValue(r5, "*");
        t4[e4] = n3 ? o2.wrapPointer(n3, o2.PeParameter) : null, r5 += Int32Array.BYTES_PER_ELEMENT;
      }
      return t4;
    });
  t3(o2.PeHorizon.prototype, "getCoord", (e3) => function() {
    const t4 = this.getSize();
    if (!t4)
      return null;
    const o5 = [];
    return A(o5, t4, e3.call(this)), o5;
  }), t3(o2.PeGTlistExtendedEntry.prototype, "getEntries", (e3) => {
    const t4 = o2._pe_getPeGTlistExtendedGTsSize();
    return function() {
      let r5 = null;
      const n3 = e3.call(this);
      if (!o2.compare(n3, o2.NULL)) {
        r5 = [n3];
        const e4 = this.getSteps();
        if (e4 > 1) {
          const _3 = o2.getPointer(n3);
          for (let n4 = 1; n4 < e4; n4++)
            r5.push(o2.wrapPointer(_3 + t4 * n4, o2.PeGTlistExtendedGTs));
        }
      }
      return r5;
    };
  });
  const n2 = o2._pe_getPeHorizonSize(), _2 = (e3) => function() {
    let t4 = this._cache;
    if (t4 || (t4 = /* @__PURE__ */ new Map(), this._cache = t4), t4.has(e3))
      return t4.get(e3);
    let r5 = null;
    const _3 = e3.call(this);
    if (!o2.compare(_3, o2.NULL)) {
      r5 = [_3];
      const e4 = _3.getNump();
      if (e4 > 1) {
        const t5 = o2.getPointer(_3);
        for (let _4 = 1; _4 < e4; _4++)
          r5.push(o2.wrapPointer(t5 + n2 * _4, o2.PeHorizon));
      }
    }
    return t4.set(e3, r5), r5;
  };
  t3(o2.PeProjcs.prototype, "horizonGcsGenerate", _2), t3(o2.PeProjcs.prototype, "horizonPcsGenerate", _2), o2.PeObject.prototype.toString = function(e3 = s.PE_STR_OPTS_NONE) {
    o2.ensureCache.prepare();
    const t4 = o2.getPointer(this), r5 = o2.ensureInt8(new Array(s.PE_BUFFER_MAX));
    return o2.UTF8ToString(o2._pe_object_to_string_ext(t4, e3, r5));
  };
}
function N(e2) {
  if (!e2)
    return;
  const t3 = o2.getClass(e2);
  if (!t3)
    return;
  const r4 = o2.getCache(t3);
  if (!r4)
    return;
  const n2 = o2.getPointer(e2);
  n2 && delete r4[n2];
}
function y(e2, t3) {
  const r4 = [], n2 = new Array(t3);
  for (let _2 = 0; _2 < e2; _2++)
    r4.push(o2.ensureInt8(n2));
  return r4;
}
function M(e2) {
  let t3;
  return Array.isArray(e2[0]) ? (t3 = [], e2.forEach((e3) => {
    t3.push(e3[0], e3[1]);
  })) : t3 = e2, t3;
}
function A(e2, t3, r4, n2 = false) {
  if (n2)
    for (let _2 = 0; _2 < 2 * t3; _2++)
      e2[_2] = o2.getValue(r4 + _2 * Float64Array.BYTES_PER_ELEMENT, "double");
  else {
    const n3 = 0 === e2.length;
    for (let _2 = 0; _2 < t3; _2++)
      n3 && (e2[_2] = new Array(2)), e2[_2][0] = o2.getValue(r4, "double"), e2[_2][1] = o2.getValue(r4 + Float64Array.BYTES_PER_ELEMENT, "double"), r4 += 2 * Float64Array.BYTES_PER_ELEMENT;
  }
}
!function(e2) {
  let t3;
  function r4() {
    e2.PE_GTLIST_OPTS_COMMON = o2.PeGTlistExtended.prototype.PE_GTLIST_OPTS_COMMON, t3 = o2._pe_getPeGTlistExtendedEntrySize();
  }
  function n2(e3, r5, n3, _2, P3, s4) {
    let E4 = null;
    const i3 = new o2.PeInteger(s4);
    try {
      const p4 = o2.PeGTlistExtended.prototype.getGTlist(e3, r5, n3, _2, P3, i3);
      if ((s4 = i3.val) && (E4 = [p4], s4 > 1)) {
        const e4 = o2.getPointer(p4);
        for (let r6 = 1; r6 < s4; r6++)
          E4.push(o2.wrapPointer(e4 + t3 * r6, o2.PeGTlistExtendedEntry));
      }
    } finally {
      o2.destroy(i3);
    }
    return E4;
  }
  e2.init = r4, e2.getGTlist = n2;
}(p || (p = {})), function(e2) {
  function t3(e3) {
    if (e3 == null ? void 0 : e3.length) {
      for (const t4 of e3)
        N(t4), t4.getEntries().forEach((e4) => {
          N(e4);
          const t5 = e4.getGeogtran();
          N(t5), t5.getParameters().forEach(N), [t5.getGeogcs1(), t5.getGeogcs2()].forEach((e5) => {
            N(e5);
            const t6 = e5.getDatum();
            N(t6), N(t6.getSpheroid()), N(e5.getPrimem()), N(e5.getUnit());
          });
        });
      o2.PeGTlistExtendedEntry.prototype.Delete(e3[0]);
    }
  }
  e2.destroy = t3;
}(u || (u = {})), function(e2) {
  function t3(e3, t4, r4, n2, _2) {
    o2.ensureCache.prepare();
    const P3 = M(r4), s4 = r4 === P3, E4 = o2.ensureFloat64(P3);
    let i3 = 0;
    n2 && (i3 = o2.ensureFloat64(n2));
    const p4 = o2._pe_geog_to_geog(o2.getPointer(e3), t4, E4, i3, _2);
    return p4 && A(r4, t4, E4, s4), p4;
  }
  e2.geogToGeog = t3;
}(a2 || (a2 = {})), function(e2) {
  const t3 = (e3, t4, r5, n3, _3, P4) => {
    let E5, i4;
    switch (o2.ensureCache.prepare(), e3) {
      case "dd":
        E5 = o2._pe_geog_to_dd, i4 = s.PE_DD_MAX;
        break;
      case "ddm":
        E5 = o2._pe_geog_to_ddm, i4 = s.PE_DDM_MAX;
        break;
      case "dms":
        E5 = o2._pe_geog_to_dms, i4 = s.PE_DMS_MAX;
    }
    let p5 = 0;
    t4 && (p5 = o2.getPointer(t4));
    const u4 = M(n3), a3 = o2.ensureFloat64(u4), c3 = y(r5, i4), T4 = E5(p5, r5, a3, _3, o2.ensureInt32(c3));
    if (T4)
      for (let s4 = 0; s4 < r5; s4++)
        P4[s4] = o2.UTF8ToString(c3[s4]);
    return T4;
  }, r4 = (e3, t4, r5, n3, _3) => {
    let P4;
    switch (o2.ensureCache.prepare(), e3) {
      case "dd":
        P4 = o2._pe_dd_to_geog;
        break;
      case "ddm":
        P4 = o2._pe_ddm_to_geog;
        break;
      case "dms":
        P4 = o2._pe_dms_to_geog;
    }
    let s4 = 0;
    t4 && (s4 = o2.getPointer(t4));
    const E5 = n3.map((e4) => o2.ensureString(e4)), i4 = o2.ensureInt32(E5), p5 = o2.ensureFloat64(new Array(2 * r5)), u4 = P4(s4, r5, i4, p5);
    return u4 && A(_3, r5, p5), u4;
  };
  function n2(e3, o5, r5, n3, _3) {
    return t3("dms", e3, o5, r5, n3, _3);
  }
  function _2(e3, t4, o5, n3) {
    return r4("dms", e3, t4, o5, n3);
  }
  function P3(e3, o5, r5, n3, _3) {
    return t3("ddm", e3, o5, r5, n3, _3);
  }
  function E4(e3, t4, o5, n3) {
    return r4("ddm", e3, t4, o5, n3);
  }
  function i3(e3, o5, r5, n3, _3) {
    return t3("dd", e3, o5, r5, n3, _3);
  }
  function p4(e3, t4, o5, n3) {
    return r4("dd", e3, t4, o5, n3);
  }
  e2.geogToDms = n2, e2.dmsToGeog = _2, e2.geogToDdm = P3, e2.ddmToGeog = E4, e2.geogToDd = i3, e2.ddToGeog = p4;
}(c || (c = {})), function(e2) {
  function t3() {
    e2.PE_MGRS_STYLE_NEW = o2.PeNotationMgrs.prototype.PE_MGRS_STYLE_NEW, e2.PE_MGRS_STYLE_OLD = o2.PeNotationMgrs.prototype.PE_MGRS_STYLE_OLD, e2.PE_MGRS_STYLE_AUTO = o2.PeNotationMgrs.prototype.PE_MGRS_STYLE_AUTO, e2.PE_MGRS_180_ZONE_1_PLUS = o2.PeNotationMgrs.prototype.PE_MGRS_180_ZONE_1_PLUS, e2.PE_MGRS_ADD_SPACES = o2.PeNotationMgrs.prototype.PE_MGRS_ADD_SPACES;
  }
  function r4(e3, t4, r5, n3, _2, P3, E4) {
    o2.ensureCache.prepare();
    let i3 = 0;
    e3 && (i3 = o2.getPointer(e3));
    const p4 = M(r5), u4 = o2.ensureFloat64(p4), a3 = y(t4, s.PE_MGRS_MAX), c3 = o2.ensureInt32(a3), T4 = o2._pe_geog_to_mgrs_extended(i3, t4, u4, n3, _2, P3, c3);
    if (T4)
      for (let s4 = 0; s4 < t4; s4++)
        E4[s4] = o2.UTF8ToString(a3[s4]);
    return T4;
  }
  function n2(e3, t4, r5, n3, _2) {
    o2.ensureCache.prepare();
    let P3 = 0;
    e3 && (P3 = o2.getPointer(e3));
    const s4 = r5.map((e4) => o2.ensureString(e4)), E4 = o2.ensureInt32(s4), i3 = o2.ensureFloat64(new Array(2 * t4)), p4 = o2._pe_mgrs_to_geog_extended(P3, t4, E4, n3, i3);
    return p4 && A(_2, t4, i3), p4;
  }
  e2.init = t3, e2.geogToMgrsExtended = r4, e2.mgrsToGeogExtended = n2;
}(T || (T = {})), function(e2) {
  function t3(e3, t4, r5, n2, _2, P3, E4) {
    o2.ensureCache.prepare();
    let i3 = 0;
    e3 && (i3 = o2.getPointer(e3));
    const p4 = M(r5), u4 = o2.ensureFloat64(p4), a3 = y(t4, s.PE_MGRS_MAX), c3 = o2.ensureInt32(a3), T4 = o2._pe_geog_to_usng(i3, t4, u4, n2, _2, P3, c3);
    if (T4)
      for (let s4 = 0; s4 < t4; s4++)
        E4[s4] = o2.UTF8ToString(a3[s4]);
    return T4;
  }
  function r4(e3, t4, r5, n2) {
    o2.ensureCache.prepare();
    let _2 = 0;
    e3 && (_2 = o2.getPointer(e3));
    const P3 = r5.map((e4) => o2.ensureString(e4)), s4 = o2.ensureInt32(P3), E4 = o2.ensureFloat64(new Array(2 * t4)), i3 = o2._pe_usng_to_geog(_2, t4, s4, E4);
    return i3 && A(n2, t4, E4), i3;
  }
  e2.geogToUsng = t3, e2.usngToGeog = r4;
}(g || (g = {})), function(e2) {
  function t3() {
    e2.PE_UTM_OPTS_NONE = o2.PeNotationUtm.prototype.PE_UTM_OPTS_NONE, e2.PE_UTM_OPTS_ADD_SPACES = o2.PeNotationUtm.prototype.PE_UTM_OPTS_ADD_SPACES, e2.PE_UTM_OPTS_NS = o2.PeNotationUtm.prototype.PE_UTM_OPTS_NS;
  }
  function r4(e3, t4, r5, n3, _2) {
    o2.ensureCache.prepare();
    let P3 = 0;
    e3 && (P3 = o2.getPointer(e3));
    const E4 = M(r5), i3 = o2.ensureFloat64(E4), p4 = y(t4, s.PE_UTM_MAX), u4 = o2.ensureInt32(p4), a3 = o2._pe_geog_to_utm(P3, t4, i3, n3, u4);
    if (a3)
      for (let s4 = 0; s4 < t4; s4++)
        _2[s4] = o2.UTF8ToString(p4[s4]);
    return a3;
  }
  function n2(e3, t4, r5, n3, _2) {
    o2.ensureCache.prepare();
    let P3 = 0;
    e3 && (P3 = o2.getPointer(e3));
    const s4 = r5.map((e4) => o2.ensureString(e4)), E4 = o2.ensureInt32(s4), i3 = o2.ensureFloat64(new Array(2 * t4)), p4 = o2._pe_utm_to_geog(P3, t4, E4, n3, i3);
    return p4 && A(_2, t4, i3), p4;
  }
  e2.init = t3, e2.geogToUtm = r4, e2.utmToGeog = n2;
}(f || (f = {})), function(e2) {
  const t3 = /* @__PURE__ */ new Map();
  function r4() {
    e2.PE_PCSINFO_OPTION_NONE = o2.PePCSInfo.prototype.PE_PCSINFO_OPTION_NONE, e2.PE_PCSINFO_OPTION_DOMAIN = o2.PePCSInfo.prototype.PE_PCSINFO_OPTION_DOMAIN, e2.PE_POLE_OUTSIDE_BOUNDARY = o2.PePCSInfo.prototype.PE_POLE_OUTSIDE_BOUNDARY, e2.PE_POLE_POINT = o2.PePCSInfo.prototype.PE_POLE_POINT;
  }
  function n2(r5, n3 = e2.PE_PCSINFO_OPTION_DOMAIN) {
    let _2 = null, P3 = null;
    return t3.has(r5) && (P3 = t3.get(r5), P3[n3] && (_2 = P3[n3])), _2 || (_2 = o2.PePCSInfo.prototype.generate(r5, n3), P3 || (P3 = [], t3.set(r5, P3)), P3[n3] = _2), _2;
  }
  e2.init = r4, e2.generate = n2;
}(S || (S = {})), function(e2) {
  function t3() {
    return o2.PeVersion.prototype.version_string();
  }
  e2.versionString = t3;
}(O || (O = {}));
var d = Object.freeze(Object.defineProperty({ __proto__: null, get PeCSTransformations() {
  return P;
}, get PeDefs() {
  return s;
}, get PeFactory() {
  return E;
}, get PeGCSExtent() {
  return i;
}, get PeGTTransformations() {
  return a2;
}, get PeGTlistExtended() {
  return p;
}, get PeGTlistExtendedEntry() {
  return u;
}, get PeNotationDms() {
  return c;
}, get PeNotationMgrs() {
  return T;
}, get PeNotationUsng() {
  return g;
}, get PeNotationUtm() {
  return f;
}, get PePCSInfo() {
  return S;
}, get PeVersion() {
  return O;
}, _init: l, get _pe() {
  return o2;
}, isLoaded: r2, isSupported: n, load: _ }, Symbol.toStringTag, { value: "Module" }));

// node_modules/@arcgis/core/geometry/support/SupportedGCSWkids.js
var S2;
!function(S4) {
  S4[S4.CGCS2000 = 4490] = "CGCS2000", S4[S4.GCSMARS2000 = 104971] = "GCSMARS2000", S4[S4.GCSMARS2000_SPHERE = 104905] = "GCSMARS2000_SPHERE", S4[S4.GCSMOON2000 = 104903] = "GCSMOON2000";
}(S2 || (S2 = {}));

// node_modules/@arcgis/core/geometry/support/WKIDUnitConversion.js
var o3;
var r3 = { values: [1, 0.3048, 0.3048006096012192, 0.3047972654, 0.9143917962, 0.201166195164, 0.9143984146160287, 0.3047994715386762, 20.11676512155263, 20.11678249437587, 0.9143985307444408, 0.91439523, 0.3047997101815088, 20.1168, 20.116756, 5e4, 15e4], units: ["Meter", "Foot", "Foot_US", "Foot_Clarke", "Yard_Clarke", "Link_Clarke", "Yard_Sears", "Foot_Sears", "Chain_Sears", "Chain_Benoit_1895_B", "Yard_Indian", "Yard_Indian_1937", "Foot_Gold_Coast", "Chain", "Chain_Sears_1922_Truncated", "50_Kilometers", "150_Kilometers"], 2066: 5, 2136: 12, 2155: 2, 2157: 0, 2158: 0, 2159: 12, 2160: 12, 2204: 2, 2219: 0, 2220: 0, 2254: 2, 2255: 2, 2256: 1, 2265: 1, 2266: 1, 2267: 2, 2268: 2, 2269: 1, 2270: 1, 2271: 2, 2272: 2, 2273: 1, 2294: 0, 2295: 0, 2314: 3, 2899: 2, 2900: 2, 2901: 1, 2909: 1, 2910: 1, 2911: 2, 2912: 2, 2913: 1, 2914: 1, 2992: 1, 2993: 0, 2994: 1, 3080: 1, 3089: 2, 3090: 0, 3091: 2, 3102: 2, 3141: 0, 3142: 0, 3167: 14, 3359: 2, 3360: 0, 3361: 1, 3362: 0, 3363: 2, 3364: 0, 3365: 2, 3366: 3, 3404: 2, 3405: 0, 3406: 0, 3407: 3, 3439: 0, 3440: 0, 3479: 1, 3480: 0, 3481: 1, 3482: 0, 3483: 1, 3484: 0, 3485: 2, 3486: 0, 3487: 2, 3488: 0, 3489: 0, 3490: 2, 3491: 0, 3492: 2, 3493: 0, 3494: 2, 3495: 0, 3496: 2, 3497: 0, 3498: 2, 3499: 0, 3500: 2, 3501: 0, 3502: 2, 3503: 0, 3504: 2, 3505: 0, 3506: 2, 3507: 0, 3508: 2, 3509: 0, 3510: 2, 3511: 0, 3512: 2, 3513: 0, 3514: 0, 3515: 2, 3516: 0, 3517: 2, 3518: 0, 3519: 2, 3520: 0, 3521: 2, 3522: 0, 3523: 2, 3524: 0, 3525: 2, 3526: 0, 3527: 2, 3528: 0, 3529: 2, 3530: 0, 3531: 2, 3532: 0, 3533: 2, 3534: 0, 3535: 2, 3536: 0, 3537: 2, 3538: 0, 3539: 2, 3540: 0, 3541: 2, 3542: 0, 3543: 2, 3544: 0, 3545: 2, 3546: 0, 3547: 2, 3548: 0, 3549: 2, 3550: 0, 3551: 2, 3552: 0, 3553: 2, 3582: 2, 3583: 0, 3584: 2, 3585: 0, 3586: 2, 3587: 0, 3588: 1, 3589: 0, 3590: 1, 3591: 0, 3592: 0, 3593: 1, 3598: 2, 3599: 0, 3600: 2, 3605: 1, 3606: 0, 3607: 0, 3608: 2, 3609: 0, 3610: 2, 3611: 0, 3612: 2, 3613: 0, 3614: 2, 3615: 0, 3616: 2, 3617: 0, 3618: 2, 3619: 0, 3620: 2, 3621: 0, 3622: 2, 3623: 0, 3624: 2, 3625: 0, 3626: 2, 3627: 0, 3628: 2, 3629: 0, 3630: 2, 3631: 0, 3632: 2, 3633: 0, 3634: 1, 3635: 0, 3636: 1, 3640: 2, 3641: 0, 3642: 2, 3643: 0, 3644: 1, 3645: 0, 3646: 1, 3647: 0, 3648: 1, 3649: 0, 3650: 2, 3651: 0, 3652: 2, 3653: 0, 3654: 2, 3655: 0, 3656: 1, 3657: 0, 3658: 2, 3659: 0, 3660: 2, 3661: 0, 3662: 2, 3663: 0, 3664: 2, 3668: 2, 3669: 0, 3670: 2, 3671: 0, 3672: 2, 3673: 0, 3674: 2, 3675: 0, 3676: 1, 3677: 2, 3678: 0, 3679: 1, 3680: 2, 3681: 0, 3682: 1, 3683: 2, 3684: 0, 3685: 0, 3686: 2, 3687: 0, 3688: 2, 3689: 0, 3690: 2, 3691: 0, 3692: 2, 3696: 2, 3697: 0, 3698: 2, 3699: 0, 3700: 2, 3793: 0, 3794: 0, 3812: 0, 3854: 0, 3857: 0, 3920: 0, 3978: 0, 3979: 0, 3991: 2, 3992: 2, 4026: 0, 4037: 0, 4038: 0, 4071: 0, 4082: 0, 4083: 0, 4087: 0, 4088: 0, 4217: 2, 4414: 0, 4415: 0, 4417: 0, 4434: 0, 4437: 0, 4438: 2, 4439: 2, 4462: 0, 4467: 0, 4471: 0, 4474: 0, 4559: 0, 4647: 0, 4822: 0, 4826: 0, 4839: 0, 5018: 0, 5041: 0, 5042: 0, 5048: 0, 5221: 0, 5223: 0, 5234: 0, 5235: 0, 5243: 0, 5247: 0, 5266: 0, 5316: 0, 5320: 0, 5321: 0, 5325: 0, 5337: 0, 5361: 0, 5362: 0, 5367: 0, 5382: 0, 5383: 0, 5396: 0, 5456: 0, 5457: 0, 5469: 0, 5472: 4, 5490: 0, 5513: 0, 5514: 0, 5523: 0, 5559: 0, 5588: 1, 5589: 3, 5596: 0, 5627: 0, 5629: 0, 5641: 0, 5643: 0, 5644: 0, 5646: 2, 5654: 2, 5655: 2, 5659: 0, 5700: 0, 5825: 0, 5836: 0, 5837: 0, 5839: 0, 5842: 0, 5844: 0, 5858: 0, 5879: 0, 5880: 0, 5887: 0, 5890: 0, 6128: 1, 6129: 1, 6141: 1, 6204: 0, 6210: 0, 6211: 0, 6307: 0, 6312: 0, 6316: 0, 6362: 0, 6391: 1, 6405: 1, 6406: 0, 6407: 1, 6408: 0, 6409: 1, 6410: 0, 6411: 2, 6412: 0, 6413: 2, 6414: 0, 6415: 0, 6416: 2, 6417: 0, 6418: 2, 6419: 0, 6420: 2, 6421: 0, 6422: 2, 6423: 0, 6424: 2, 6425: 0, 6426: 2, 6427: 0, 6428: 2, 6429: 0, 6430: 2, 6431: 0, 6432: 2, 6433: 0, 6434: 2, 6435: 0, 6436: 2, 6437: 0, 6438: 2, 6439: 0, 6440: 0, 6441: 2, 6442: 0, 6443: 2, 6444: 0, 6445: 2, 6446: 0, 6447: 2, 6448: 0, 6449: 2, 6450: 0, 6451: 2, 6452: 0, 6453: 2, 6454: 0, 6455: 2, 6456: 0, 6457: 2, 6458: 0, 6459: 2, 6460: 0, 6461: 2, 6462: 0, 6463: 2, 6464: 0, 6465: 2, 6466: 0, 6467: 2, 6468: 0, 6469: 2, 6470: 0, 6471: 2, 6472: 0, 6473: 2, 6474: 0, 6475: 2, 6476: 0, 6477: 2, 6478: 0, 6479: 2, 6484: 2, 6485: 0, 6486: 2, 6487: 0, 6488: 2, 6489: 0, 6490: 2, 6491: 0, 6492: 2, 6493: 0, 6494: 1, 6495: 0, 6496: 1, 6497: 0, 6498: 0, 6499: 1, 6500: 0, 6501: 2, 6502: 0, 6503: 2, 6504: 0, 6505: 2, 6506: 0, 6507: 2, 6508: 0, 6509: 0, 6510: 2, 6515: 1, 6516: 0, 6518: 0, 6519: 2, 6520: 0, 6521: 2, 6522: 0, 6523: 2, 6524: 0, 6525: 2, 6526: 0, 6527: 2, 6528: 0, 6529: 2, 6530: 0, 6531: 2, 6532: 0, 6533: 2, 6534: 0, 6535: 2, 6536: 0, 6537: 2, 6538: 0, 6539: 2, 6540: 0, 6541: 2, 6542: 0, 6543: 2, 6544: 0, 6545: 1, 6546: 0, 6547: 1, 6548: 0, 6549: 2, 6550: 0, 6551: 2, 6552: 0, 6553: 2, 6554: 0, 6555: 2, 6556: 0, 6557: 1, 6558: 0, 6559: 1, 6560: 0, 6561: 1, 6562: 0, 6563: 2, 6564: 0, 6565: 2, 6566: 0, 6567: 0, 6568: 2, 6569: 0, 6570: 1, 6571: 0, 6572: 2, 6573: 0, 6574: 2, 6575: 0, 6576: 2, 6577: 0, 6578: 2, 6582: 2, 6583: 0, 6584: 2, 6585: 0, 6586: 2, 6587: 0, 6588: 2, 6589: 0, 6590: 2, 6591: 0, 6592: 0, 6593: 2, 6594: 0, 6595: 2, 6596: 0, 6597: 2, 6598: 0, 6599: 2, 6600: 0, 6601: 2, 6602: 0, 6603: 2, 6605: 2, 6606: 0, 6607: 2, 6608: 0, 6609: 2, 6610: 0, 6611: 0, 6612: 2, 6613: 0, 6614: 2, 6615: 0, 6616: 2, 6617: 0, 6618: 2, 6633: 2, 6646: 0, 6703: 0, 6784: 0, 6785: 1, 6786: 0, 6787: 1, 6788: 0, 6789: 1, 6790: 0, 6791: 1, 6792: 0, 6793: 1, 6794: 0, 6795: 1, 6796: 0, 6797: 1, 6798: 0, 6799: 1, 6800: 0, 6801: 1, 6802: 0, 6803: 1, 6804: 0, 6805: 1, 6806: 0, 6807: 1, 6808: 0, 6809: 1, 6810: 0, 6811: 1, 6812: 0, 6813: 1, 6814: 0, 6815: 1, 6816: 0, 6817: 1, 6818: 0, 6819: 1, 6820: 0, 6821: 1, 6822: 0, 6823: 1, 6824: 0, 6825: 1, 6826: 0, 6827: 1, 6828: 0, 6829: 1, 6830: 0, 6831: 1, 6832: 0, 6833: 1, 6834: 0, 6835: 1, 6836: 0, 6837: 1, 6838: 0, 6839: 1, 6840: 0, 6841: 1, 6842: 0, 6843: 1, 6844: 0, 6845: 1, 6846: 0, 6847: 1, 6848: 0, 6849: 1, 6850: 0, 6851: 1, 6852: 0, 6853: 1, 6854: 0, 6855: 1, 6856: 0, 6857: 1, 6858: 0, 6859: 1, 6860: 0, 6861: 1, 6862: 0, 6863: 1, 6867: 0, 6868: 1, 6870: 0, 6875: 0, 6876: 0, 6879: 0, 6880: 2, 6884: 0, 6885: 1, 6886: 0, 6887: 1, 6915: 0, 6922: 0, 6923: 2, 6924: 0, 6925: 2, 6962: 0, 6984: 0, 6991: 0, 7128: 2, 7131: 0, 7132: 2, 7142: 0, 7257: 0, 7258: 2, 7259: 0, 7260: 2, 7261: 0, 7262: 2, 7263: 0, 7264: 2, 7265: 0, 7266: 2, 7267: 0, 7268: 2, 7269: 0, 7270: 2, 7271: 0, 7272: 2, 7273: 0, 7274: 2, 7275: 0, 7276: 2, 7277: 0, 7278: 2, 7279: 0, 7280: 2, 7281: 0, 7282: 2, 7283: 0, 7284: 2, 7285: 0, 7286: 2, 7287: 0, 7288: 2, 7289: 0, 7290: 2, 7291: 0, 7292: 2, 7293: 0, 7294: 2, 7295: 0, 7296: 2, 7297: 0, 7298: 2, 7299: 0, 7300: 2, 7301: 0, 7302: 2, 7303: 0, 7304: 2, 7305: 0, 7306: 2, 7307: 0, 7308: 2, 7309: 0, 7310: 2, 7311: 0, 7312: 2, 7313: 0, 7314: 2, 7315: 0, 7316: 2, 7317: 0, 7318: 2, 7319: 0, 7320: 2, 7321: 0, 7322: 2, 7323: 0, 7324: 2, 7325: 0, 7326: 2, 7327: 0, 7328: 2, 7329: 0, 7330: 2, 7331: 0, 7332: 2, 7333: 0, 7334: 2, 7335: 0, 7336: 2, 7337: 0, 7338: 2, 7339: 0, 7340: 2, 7341: 0, 7342: 2, 7343: 0, 7344: 2, 7345: 0, 7346: 2, 7347: 0, 7348: 2, 7349: 0, 7350: 2, 7351: 0, 7352: 2, 7353: 0, 7354: 2, 7355: 0, 7356: 2, 7357: 0, 7358: 2, 7359: 0, 7360: 2, 7361: 0, 7362: 2, 7363: 0, 7364: 2, 7365: 0, 7366: 2, 7367: 0, 7368: 2, 7369: 0, 7370: 2, 7877: 0, 7878: 0, 7882: 0, 7883: 0, 7887: 0, 7899: 0, 7991: 0, 7992: 0, 8035: 2, 8036: 2, 8058: 0, 8059: 0, 8082: 0, 8083: 0, 8088: 0, 8090: 0, 8091: 2, 8092: 0, 8093: 2, 8095: 0, 8096: 2, 8097: 0, 8098: 2, 8099: 0, 8100: 2, 8101: 0, 8102: 2, 8103: 0, 8104: 2, 8105: 0, 8106: 2, 8107: 0, 8108: 2, 8109: 0, 8110: 2, 8111: 0, 8112: 2, 8113: 0, 8114: 2, 8115: 0, 8116: 2, 8117: 0, 8118: 2, 8119: 0, 8120: 2, 8121: 0, 8122: 2, 8123: 0, 8124: 2, 8125: 0, 8126: 2, 8127: 0, 8128: 2, 8129: 0, 8130: 2, 8131: 0, 8132: 2, 8133: 0, 8134: 2, 8135: 0, 8136: 2, 8137: 0, 8138: 2, 8139: 0, 8140: 2, 8141: 0, 8142: 2, 8143: 0, 8144: 2, 8145: 0, 8146: 2, 8147: 0, 8148: 2, 8149: 0, 8150: 2, 8151: 0, 8152: 2, 8153: 0, 8154: 2, 8155: 0, 8156: 2, 8157: 0, 8158: 2, 8159: 0, 8160: 2, 8161: 0, 8162: 2, 8163: 0, 8164: 2, 8165: 0, 8166: 2, 8167: 0, 8168: 2, 8169: 0, 8170: 2, 8171: 0, 8172: 2, 8173: 0, 8177: 2, 8179: 0, 8180: 2, 8181: 0, 8182: 2, 8184: 0, 8185: 2, 8187: 0, 8189: 2, 8191: 0, 8193: 2, 8196: 0, 8197: 2, 8198: 0, 8200: 2, 8201: 0, 8202: 2, 8203: 0, 8204: 2, 8205: 0, 8206: 2, 8207: 0, 8208: 2, 8209: 0, 8210: 2, 8212: 0, 8213: 2, 8214: 0, 8216: 2, 8218: 0, 8220: 2, 8222: 0, 8224: 2, 8225: 0, 8226: 2, 8311: 0, 8312: 1, 8313: 0, 8314: 1, 8315: 0, 8316: 1, 8317: 0, 8318: 1, 8319: 0, 8320: 1, 8321: 0, 8322: 1, 8323: 0, 8324: 1, 8325: 0, 8326: 1, 8327: 0, 8328: 1, 8329: 0, 8330: 1, 8331: 0, 8332: 1, 8333: 0, 8334: 1, 8335: 0, 8336: 1, 8337: 0, 8338: 1, 8339: 0, 8340: 1, 8341: 0, 8342: 1, 8343: 0, 8344: 1, 8345: 0, 8346: 1, 8347: 0, 8348: 1, 8352: 0, 8353: 0, 8379: 0, 8380: 2, 8381: 0, 8382: 2, 8383: 0, 8384: 2, 8385: 0, 8387: 2, 8391: 0, 8395: 0, 8433: 0, 8441: 0, 8455: 0, 8456: 0, 8531: 2, 8682: 0, 8686: 0, 8687: 0, 8692: 0, 8693: 0, 8826: 0, 8903: 0, 8950: 0, 8951: 0, 9039: 0, 9040: 0, 9141: 0, 9149: 0, 9150: 0, 9191: 0, 9221: 0, 9222: 0, 9249: 0, 9250: 0, 9252: 0, 9254: 0, 9265: 0, 9284: 0, 9285: 0, 9300: 0, 9354: 0, 9367: 0, 9373: 0, 9377: 0, 9387: 0, 9391: 0, 9456: 0, 9473: 0, 9498: 0, 9674: 0, 9678: 0, 9680: 0, 9709: 0, 9712: 0, 9713: 0, 9716: 0, 9741: 0, 9748: 2, 9749: 2, 9761: 0, 9766: 0, 9793: 0, 9794: 0, 9869: 0, 9874: 0, 9875: 0, 9880: 0, 9895: 0, 9943: 0, 9945: 0, 9947: 0, 9967: 0, 9972: 0, 9977: 0, 10160: 0, 10183: 0, 10188: 0, 10194: 0, 10199: 0, 10207: 0, 10212: 0, 10217: 0, 10222: 0, 10227: 0, 10235: 0, 10240: 0, 10250: 0, 10254: 0, 10270: 0, 10275: 0, 10280: 0, 10306: 0, 10329: 0, 10471: 0, 20002: 0, 20042: 0, 20050: 1, 20499: 0, 20538: 0, 20539: 0, 20790: 0, 20791: 0, 21291: 0, 21292: 0, 21500: 0, 21817: 0, 21818: 0, 22032: 0, 22033: 0, 22091: 0, 22092: 0, 22239: 0, 22240: 0, 22332: 0, 22337: 0, 22338: 0, 22391: 0, 22392: 0, 22639: 0, 22700: 0, 22739: 0, 22770: 0, 22780: 0, 22832: 0, 23090: 0, 23095: 0, 23239: 0, 23240: 0, 23433: 0, 23700: 0, 24047: 0, 24048: 0, 24100: 3, 24200: 0, 24305: 0, 24306: 0, 24382: 10, 24383: 0, 24500: 0, 24547: 0, 24548: 0, 24571: 9, 24600: 0, 25e3: 0, 25231: 0, 25884: 0, 25932: 0, 26237: 0, 26331: 0, 26332: 0, 26432: 0, 26591: 0, 26592: 0, 26632: 0, 26692: 0, 27120: 0, 27200: 0, 27291: 6, 27292: 6, 27429: 0, 27492: 0, 27493: 0, 27500: 0, 27700: 0, 28232: 0, 28600: 0, 28991: 0, 28992: 0, 29100: 0, 29101: 0, 29220: 0, 29221: 0, 29333: 0, 29635: 0, 29636: 0, 29701: 0, 29738: 0, 29739: 0, 29849: 0, 29850: 0, 29871: 8, 29872: 7, 29873: 0, 29874: 0, 30200: 5, 30339: 0, 30340: 0, 30591: 0, 30592: 0, 30791: 0, 30792: 0, 30800: 0, 31028: 0, 31121: 0, 31154: 0, 31170: 0, 31171: 0, 31370: 0, 31528: 0, 31529: 0, 31600: 0, 31700: 0, 31838: 0, 31839: 0, 31900: 0, 31901: 0, 32061: 0, 32062: 0, 32098: 0, 32099: 2, 32100: 0, 32104: 0, 32161: 0, 32766: 0, 53048: 0, 53049: 0, 54090: 0, 54091: 0, 65061: 2, 65062: 2, 65161: 0, 65163: 0, 102041: 2, 102064: 11, 102068: 15, 102069: 16, 102118: 2, 102119: 1, 102120: 2, 102121: 2, 102217: 2, 102218: 0, 102219: 2, 102220: 2, 102378: 1, 102379: 1, 102380: 0, 102381: 1, 102589: 2, 102599: 2, 102600: 2, 102604: 2, 102647: 0, 102704: 2, 102705: 2, 102706: 0, 102731: 0, 102732: 0, 102759: 1, 102760: 1, 102761: 2, 102762: 0, 102763: 2, 102764: 0, 102765: 0, 102766: 2, 102970: 1, 102974: 2, 102993: 0, 102994: 0, 102995: 2, 102996: 2, 103015: 0, 103016: 2, 103017: 0, 103018: 2, 103025: 0, 103026: 0, 103027: 2, 103028: 2, 103035: 0, 103036: 0, 103037: 2, 103038: 2, 103039: 0, 103040: 0, 103041: 2, 103042: 2, 103043: 0, 103044: 0, 103045: 2, 103046: 2, 103047: 0, 103048: 0, 103049: 2, 103050: 2, 103051: 0, 103052: 2, 103053: 0, 103054: 2, 103055: 0, 103056: 2, 103057: 0, 103058: 0, 103059: 2, 103060: 2, 103061: 0, 103062: 0, 103063: 2, 103064: 2, 103069: 2, 103070: 0, 103071: 0, 103072: 2, 103073: 2, 103086: 0, 103087: 0, 103088: 2, 103089: 2, 103094: 1, 103095: 0, 103096: 2, 103103: 0, 103104: 2, 103105: 0, 103106: 2, 103121: 0, 103122: 2, 103123: 0, 103124: 0, 103125: 1, 103126: 1, 103127: 0, 103128: 0, 103129: 2, 103130: 2, 103131: 0, 103132: 0, 103133: 2, 103134: 2, 103135: 0, 103136: 0, 103137: 1, 103138: 1, 103139: 0, 103140: 2, 103141: 0, 103142: 2, 103143: 0, 103144: 2, 103145: 0, 103146: 1, 103147: 0, 103148: 0, 103149: 2, 103150: 2, 103151: 0, 103152: 2, 103172: 0, 103173: 2, 103174: 0, 103175: 0, 103176: 2, 103177: 2, 103178: 0, 103179: 0, 103180: 2, 103181: 2, 103182: 0, 103183: 0, 103184: 2, 103185: 2, 103228: 0, 103229: 0, 103230: 2, 103231: 2, 103250: 0, 103251: 2, 103252: 0, 103253: 2, 103260: 0, 103261: 0, 103262: 2, 103263: 2, 103270: 0, 103271: 0, 103272: 2, 103273: 2, 103274: 0, 103275: 0, 103276: 2, 103277: 2, 103278: 0, 103279: 0, 103280: 2, 103281: 2, 103282: 0, 103283: 0, 103284: 2, 103285: 2, 103286: 0, 103287: 2, 103288: 0, 103289: 2, 103290: 0, 103291: 2, 103292: 0, 103293: 0, 103294: 2, 103295: 2, 103296: 0, 103297: 0, 103298: 2, 103299: 2, 103376: 2, 103377: 0, 103378: 0, 103379: 2, 103380: 2, 103393: 0, 103394: 0, 103395: 2, 103396: 2, 103472: 0, 103473: 1, 103474: 0, 103475: 2, 103482: 0, 103483: 2, 103484: 0, 103485: 2, 103500: 0, 103501: 2, 103502: 0, 103503: 0, 103504: 1, 103505: 1, 103506: 0, 103507: 0, 103508: 2, 103509: 2, 103510: 0, 103511: 0, 103512: 2, 103513: 2, 103514: 0, 103515: 2, 103516: 0, 103517: 2, 103518: 0, 103519: 2, 103520: 0, 103521: 1, 103522: 0, 103523: 0, 103524: 2, 103525: 2, 103526: 0, 103527: 2, 103561: 2, 103562: 2, 103563: 0, 103564: 0, 103565: 2, 103566: 2, 103567: 0, 103568: 0, 103569: 2, 103570: 2, 103584: 0, 103585: 2, 103586: 0, 103587: 2, 103588: 1, 103589: 0, 103590: 2, 103591: 1, 103592: 0, 103593: 2, 103594: 1, 103695: 2, 103897: 1 };
for (o3 = 2e3; o3 <= 2045; o3++)
  r3[o3] = 0;
for (o3 = 2056; o3 <= 2065; o3++)
  r3[o3] = 0;
for (o3 = 2067; o3 <= 2135; o3++)
  r3[o3] = 0;
for (o3 = 2137; o3 <= 2154; o3++)
  r3[o3] = 0;
for (o3 = 2161; o3 <= 2170; o3++)
  r3[o3] = 0;
for (o3 = 2172; o3 <= 2193; o3++)
  r3[o3] = 0;
for (o3 = 2195; o3 <= 2198; o3++)
  r3[o3] = 0;
for (o3 = 2200; o3 <= 2203; o3++)
  r3[o3] = 0;
for (o3 = 2205; o3 <= 2217; o3++)
  r3[o3] = 0;
for (o3 = 2222; o3 <= 2224; o3++)
  r3[o3] = 1;
for (o3 = 2225; o3 <= 2250; o3++)
  r3[o3] = 2;
for (o3 = 2251; o3 <= 2253; o3++)
  r3[o3] = 1;
for (o3 = 2257; o3 <= 2264; o3++)
  r3[o3] = 2;
for (o3 = 2274; o3 <= 2279; o3++)
  r3[o3] = 2;
for (o3 = 2280; o3 <= 2282; o3++)
  r3[o3] = 1;
for (o3 = 2283; o3 <= 2289; o3++)
  r3[o3] = 2;
for (o3 = 2290; o3 <= 2292; o3++)
  r3[o3] = 0;
for (o3 = 2308; o3 <= 2313; o3++)
  r3[o3] = 0;
for (o3 = 2315; o3 <= 2491; o3++)
  r3[o3] = 0;
for (o3 = 2494; o3 <= 2866; o3++)
  r3[o3] = 0;
for (o3 = 2867; o3 <= 2869; o3++)
  r3[o3] = 1;
for (o3 = 2870; o3 <= 2888; o3++)
  r3[o3] = 2;
for (o3 = 2891; o3 <= 2895; o3++)
  r3[o3] = 2;
for (o3 = 2896; o3 <= 2898; o3++)
  r3[o3] = 1;
for (o3 = 2902; o3 <= 2908; o3++)
  r3[o3] = 2;
for (o3 = 2915; o3 <= 2920; o3++)
  r3[o3] = 2;
for (o3 = 2921; o3 <= 2923; o3++)
  r3[o3] = 1;
for (o3 = 2924; o3 <= 2930; o3++)
  r3[o3] = 2;
for (o3 = 2931; o3 <= 2962; o3++)
  r3[o3] = 0;
for (o3 = 2964; o3 <= 2968; o3++)
  r3[o3] = 2;
for (o3 = 2969; o3 <= 2973; o3++)
  r3[o3] = 0;
for (o3 = 2975; o3 <= 2991; o3++)
  r3[o3] = 0;
for (o3 = 2995; o3 <= 3051; o3++)
  r3[o3] = 0;
for (o3 = 3054; o3 <= 3079; o3++)
  r3[o3] = 0;
for (o3 = 3081; o3 <= 3088; o3++)
  r3[o3] = 0;
for (o3 = 3092; o3 <= 3101; o3++)
  r3[o3] = 0;
for (o3 = 3106; o3 <= 3138; o3++)
  r3[o3] = 0;
for (o3 = 3146; o3 <= 3151; o3++)
  r3[o3] = 0;
for (o3 = 3153; o3 <= 3166; o3++)
  r3[o3] = 0;
for (o3 = 3168; o3 <= 3172; o3++)
  r3[o3] = 0;
for (o3 = 3174; o3 <= 3203; o3++)
  r3[o3] = 0;
for (o3 = 3294; o3 <= 3358; o3++)
  r3[o3] = 0;
for (o3 = 3367; o3 <= 3403; o3++)
  r3[o3] = 0;
for (o3 = 3408; o3 <= 3416; o3++)
  r3[o3] = 0;
for (o3 = 3417; o3 <= 3438; o3++)
  r3[o3] = 2;
for (o3 = 3441; o3 <= 3446; o3++)
  r3[o3] = 2;
for (o3 = 3447; o3 <= 3450; o3++)
  r3[o3] = 0;
for (o3 = 3451; o3 <= 3459; o3++)
  r3[o3] = 2;
for (o3 = 3460; o3 <= 3478; o3++)
  r3[o3] = 0;
for (o3 = 3554; o3 <= 3559; o3++)
  r3[o3] = 0;
for (o3 = 3560; o3 <= 3570; o3++)
  r3[o3] = 2;
for (o3 = 3571; o3 <= 3581; o3++)
  r3[o3] = 0;
for (o3 = 3594; o3 <= 3597; o3++)
  r3[o3] = 0;
for (o3 = 3601; o3 <= 3604; o3++)
  r3[o3] = 0;
for (o3 = 3637; o3 <= 3639; o3++)
  r3[o3] = 0;
for (o3 = 3665; o3 <= 3667; o3++)
  r3[o3] = 0;
for (o3 = 3693; o3 <= 3695; o3++)
  r3[o3] = 0;
for (o3 = 3701; o3 <= 3727; o3++)
  r3[o3] = 0;
for (o3 = 3728; o3 <= 3739; o3++)
  r3[o3] = 2;
for (o3 = 3740; o3 <= 3751; o3++)
  r3[o3] = 0;
for (o3 = 3753; o3 <= 3760; o3++)
  r3[o3] = 2;
for (o3 = 3761; o3 <= 3773; o3++)
  r3[o3] = 0;
for (o3 = 3775; o3 <= 3777; o3++)
  r3[o3] = 0;
for (o3 = 3779; o3 <= 3781; o3++)
  r3[o3] = 0;
for (o3 = 3783; o3 <= 3785; o3++)
  r3[o3] = 0;
for (o3 = 3788; o3 <= 3791; o3++)
  r3[o3] = 0;
for (o3 = 3797; o3 <= 3802; o3++)
  r3[o3] = 0;
for (o3 = 3814; o3 <= 3816; o3++)
  r3[o3] = 0;
for (o3 = 3825; o3 <= 3829; o3++)
  r3[o3] = 0;
for (o3 = 3832; o3 <= 3841; o3++)
  r3[o3] = 0;
for (o3 = 3844; o3 <= 3852; o3++)
  r3[o3] = 0;
for (o3 = 3873; o3 <= 3885; o3++)
  r3[o3] = 0;
for (o3 = 3890; o3 <= 3893; o3++)
  r3[o3] = 0;
for (o3 = 3907; o3 <= 3912; o3++)
  r3[o3] = 0;
for (o3 = 3942; o3 <= 3950; o3++)
  r3[o3] = 0;
for (o3 = 3968; o3 <= 3970; o3++)
  r3[o3] = 0;
for (o3 = 3973; o3 <= 3976; o3++)
  r3[o3] = 0;
for (o3 = 3986; o3 <= 3989; o3++)
  r3[o3] = 0;
for (o3 = 3994; o3 <= 3997; o3++)
  r3[o3] = 0;
for (o3 = 4048; o3 <= 4051; o3++)
  r3[o3] = 0;
for (o3 = 4056; o3 <= 4063; o3++)
  r3[o3] = 0;
for (o3 = 4093; o3 <= 4096; o3++)
  r3[o3] = 0;
for (o3 = 4390; o3 <= 4398; o3++)
  r3[o3] = 0;
for (o3 = 4399; o3 <= 4413; o3++)
  r3[o3] = 2;
for (o3 = 4418; o3 <= 4433; o3++)
  r3[o3] = 2;
for (o3 = 4455; o3 <= 4457; o3++)
  r3[o3] = 2;
for (o3 = 4484; o3 <= 4489; o3++)
  r3[o3] = 0;
for (o3 = 4491; o3 <= 4554; o3++)
  r3[o3] = 0;
for (o3 = 4568; o3 <= 4589; o3++)
  r3[o3] = 0;
for (o3 = 4652; o3 <= 4656; o3++)
  r3[o3] = 0;
for (o3 = 4766; o3 <= 4800; o3++)
  r3[o3] = 0;
for (o3 = 5014; o3 <= 5016; o3++)
  r3[o3] = 0;
for (o3 = 5069; o3 <= 5072; o3++)
  r3[o3] = 0;
for (o3 = 5105; o3 <= 5130; o3++)
  r3[o3] = 0;
for (o3 = 5167; o3 <= 5188; o3++)
  r3[o3] = 0;
for (o3 = 5253; o3 <= 5259; o3++)
  r3[o3] = 0;
for (o3 = 5269; o3 <= 5275; o3++)
  r3[o3] = 0;
for (o3 = 5292; o3 <= 5311; o3++)
  r3[o3] = 0;
for (o3 = 5329; o3 <= 5331; o3++)
  r3[o3] = 0;
for (o3 = 5343; o3 <= 5349; o3++)
  r3[o3] = 0;
for (o3 = 5355; o3 <= 5357; o3++)
  r3[o3] = 0;
for (o3 = 5387; o3 <= 5389; o3++)
  r3[o3] = 0;
for (o3 = 5459; o3 <= 5463; o3++)
  r3[o3] = 0;
for (o3 = 5479; o3 <= 5482; o3++)
  r3[o3] = 0;
for (o3 = 5518; o3 <= 5520; o3++)
  r3[o3] = 0;
for (o3 = 5530; o3 <= 5539; o3++)
  r3[o3] = 0;
for (o3 = 5550; o3 <= 5552; o3++)
  r3[o3] = 0;
for (o3 = 5562; o3 <= 5583; o3++)
  r3[o3] = 0;
for (o3 = 5623; o3 <= 5625; o3++)
  r3[o3] = 2;
for (o3 = 5631; o3 <= 5639; o3++)
  r3[o3] = 0;
for (o3 = 5649; o3 <= 5653; o3++)
  r3[o3] = 0;
for (o3 = 5663; o3 <= 5680; o3++)
  r3[o3] = 0;
for (o3 = 5682; o3 <= 5685; o3++)
  r3[o3] = 0;
for (o3 = 5875; o3 <= 5877; o3++)
  r3[o3] = 0;
for (o3 = 5896; o3 <= 5899; o3++)
  r3[o3] = 0;
for (o3 = 5921; o3 <= 5940; o3++)
  r3[o3] = 0;
for (o3 = 6050; o3 <= 6125; o3++)
  r3[o3] = 0;
for (o3 = 6244; o3 <= 6275; o3++)
  r3[o3] = 0;
for (o3 = 6328; o3 <= 6348; o3++)
  r3[o3] = 0;
for (o3 = 6350; o3 <= 6356; o3++)
  r3[o3] = 0;
for (o3 = 6366; o3 <= 6372; o3++)
  r3[o3] = 0;
for (o3 = 6381; o3 <= 6387; o3++)
  r3[o3] = 0;
for (o3 = 6393; o3 <= 6404; o3++)
  r3[o3] = 0;
for (o3 = 6480; o3 <= 6483; o3++)
  r3[o3] = 0;
for (o3 = 6511; o3 <= 6514; o3++)
  r3[o3] = 0;
for (o3 = 6579; o3 <= 6581; o3++)
  r3[o3] = 0;
for (o3 = 6619; o3 <= 6624; o3++)
  r3[o3] = 0;
for (o3 = 6625; o3 <= 6627; o3++)
  r3[o3] = 2;
for (o3 = 6628; o3 <= 6632; o3++)
  r3[o3] = 0;
for (o3 = 6634; o3 <= 6637; o3++)
  r3[o3] = 0;
for (o3 = 6669; o3 <= 6692; o3++)
  r3[o3] = 0;
for (o3 = 6707; o3 <= 6709; o3++)
  r3[o3] = 0;
for (o3 = 6720; o3 <= 6723; o3++)
  r3[o3] = 0;
for (o3 = 6732; o3 <= 6738; o3++)
  r3[o3] = 0;
for (o3 = 6931; o3 <= 6933; o3++)
  r3[o3] = 0;
for (o3 = 6956; o3 <= 6959; o3++)
  r3[o3] = 0;
for (o3 = 7005; o3 <= 7007; o3++)
  r3[o3] = 0;
for (o3 = 7057; o3 <= 7070; o3++)
  r3[o3] = 2;
for (o3 = 7074; o3 <= 7082; o3++)
  r3[o3] = 0;
for (o3 = 7109; o3 <= 7118; o3++)
  r3[o3] = 0;
for (o3 = 7119; o3 <= 7127; o3++)
  r3[o3] = 1;
for (o3 = 7374; o3 <= 7376; o3++)
  r3[o3] = 0;
for (o3 = 7528; o3 <= 7586; o3++)
  r3[o3] = 0;
for (o3 = 7587; o3 <= 7645; o3++)
  r3[o3] = 2;
for (o3 = 7692; o3 <= 7696; o3++)
  r3[o3] = 0;
for (o3 = 7755; o3 <= 7787; o3++)
  r3[o3] = 0;
for (o3 = 7791; o3 <= 7795; o3++)
  r3[o3] = 0;
for (o3 = 7799; o3 <= 7801; o3++)
  r3[o3] = 0;
for (o3 = 7803; o3 <= 7805; o3++)
  r3[o3] = 0;
for (o3 = 7825; o3 <= 7831; o3++)
  r3[o3] = 0;
for (o3 = 7845; o3 <= 7859; o3++)
  r3[o3] = 0;
for (o3 = 8013; o3 <= 8032; o3++)
  r3[o3] = 0;
for (o3 = 8065; o3 <= 8068; o3++)
  r3[o3] = 1;
for (o3 = 8518; o3 <= 8529; o3++)
  r3[o3] = 2;
for (o3 = 8533; o3 <= 8536; o3++)
  r3[o3] = 2;
for (o3 = 8538; o3 <= 8540; o3++)
  r3[o3] = 2;
for (o3 = 8677; o3 <= 8679; o3++)
  r3[o3] = 0;
for (o3 = 8836; o3 <= 8840; o3++)
  r3[o3] = 0;
for (o3 = 8857; o3 <= 8859; o3++)
  r3[o3] = 0;
for (o3 = 8908; o3 <= 8910; o3++)
  r3[o3] = 0;
for (o3 = 9154; o3 <= 9159; o3++)
  r3[o3] = 0;
for (o3 = 9205; o3 <= 9218; o3++)
  r3[o3] = 0;
for (o3 = 9271; o3 <= 9273; o3++)
  r3[o3] = 0;
for (o3 = 9295; o3 <= 9297; o3++)
  r3[o3] = 0;
for (o3 = 9356; o3 <= 9360; o3++)
  r3[o3] = 0;
for (o3 = 9404; o3 <= 9407; o3++)
  r3[o3] = 0;
for (o3 = 9476; o3 <= 9482; o3++)
  r3[o3] = 0;
for (o3 = 9487; o3 <= 9494; o3++)
  r3[o3] = 0;
for (o3 = 9697; o3 <= 9699; o3++)
  r3[o3] = 0;
for (o3 = 9821; o3 <= 9865; o3++)
  r3[o3] = 0;
for (o3 = 10285; o3 <= 10291; o3++)
  r3[o3] = 0;
for (o3 = 10314; o3 <= 10317; o3++)
  r3[o3] = 0;
for (o3 = 10448; o3 <= 10465; o3++)
  r3[o3] = 0;
for (o3 = 11114; o3 <= 11118; o3++)
  r3[o3] = 0;
for (o3 = 20004; o3 <= 20032; o3++)
  r3[o3] = 0;
for (o3 = 20047; o3 <= 20049; o3++)
  r3[o3] = 0;
for (o3 = 20062; o3 <= 20092; o3++)
  r3[o3] = 0;
for (o3 = 20135; o3 <= 20138; o3++)
  r3[o3] = 0;
for (o3 = 20248; o3 <= 20258; o3++)
  r3[o3] = 0;
for (o3 = 20348; o3 <= 20358; o3++)
  r3[o3] = 0;
for (o3 = 20436; o3 <= 20440; o3++)
  r3[o3] = 0;
for (o3 = 20822; o3 <= 20824; o3++)
  r3[o3] = 0;
for (o3 = 20904; o3 <= 20932; o3++)
  r3[o3] = 0;
for (o3 = 20934; o3 <= 20936; o3++)
  r3[o3] = 0;
for (o3 = 21004; o3 <= 21032; o3++)
  r3[o3] = 0;
for (o3 = 21035; o3 <= 21037; o3++)
  r3[o3] = 0;
for (o3 = 21095; o3 <= 21097; o3++)
  r3[o3] = 0;
for (o3 = 21148; o3 <= 21150; o3++)
  r3[o3] = 0;
for (o3 = 21207; o3 <= 21264; o3++)
  r3[o3] = 0;
for (o3 = 21307; o3 <= 21364; o3++)
  r3[o3] = 0;
for (o3 = 21413; o3 <= 21423; o3++)
  r3[o3] = 0;
for (o3 = 21453; o3 <= 21463; o3++)
  r3[o3] = 0;
for (o3 = 21473; o3 <= 21483; o3++)
  r3[o3] = 0;
for (o3 = 21780; o3 <= 21782; o3++)
  r3[o3] = 0;
for (o3 = 21891; o3 <= 21894; o3++)
  r3[o3] = 0;
for (o3 = 21896; o3 <= 21899; o3++)
  r3[o3] = 0;
for (o3 = 22171; o3 <= 22177; o3++)
  r3[o3] = 0;
for (o3 = 22181; o3 <= 22187; o3++)
  r3[o3] = 0;
for (o3 = 22191; o3 <= 22197; o3++)
  r3[o3] = 0;
for (o3 = 22207; o3 <= 22222; o3++)
  r3[o3] = 0;
for (o3 = 22229; o3 <= 22232; o3++)
  r3[o3] = 0;
for (o3 = 22234; o3 <= 22236; o3++)
  r3[o3] = 0;
for (o3 = 22243; o3 <= 22250; o3++)
  r3[o3] = 0;
for (o3 = 22262; o3 <= 22265; o3++)
  r3[o3] = 0;
for (o3 = 22307; o3 <= 22322; o3++)
  r3[o3] = 0;
for (o3 = 22348; o3 <= 22357; o3++)
  r3[o3] = 0;
for (o3 = 22407; o3 <= 22422; o3++)
  r3[o3] = 0;
for (o3 = 22462; o3 <= 22465; o3++)
  r3[o3] = 0;
for (o3 = 22521; o3 <= 22525; o3++)
  r3[o3] = 0;
for (o3 = 22607; o3 <= 22622; o3++)
  r3[o3] = 0;
for (o3 = 22641; o3 <= 22646; o3++)
  r3[o3] = 0;
for (o3 = 22648; o3 <= 22657; o3++)
  r3[o3] = 0;
for (o3 = 22707; o3 <= 22722; o3++)
  r3[o3] = 0;
for (o3 = 22762; o3 <= 22765; o3++)
  r3[o3] = 0;
for (o3 = 22807; o3 <= 22822; o3++)
  r3[o3] = 0;
for (o3 = 22991; o3 <= 22994; o3++)
  r3[o3] = 0;
for (o3 = 23028; o3 <= 23038; o3++)
  r3[o3] = 0;
for (o3 = 23301; o3 <= 23333; o3++)
  r3[o3] = 2;
for (o3 = 23830; o3 <= 23853; o3++)
  r3[o3] = 0;
for (o3 = 23866; o3 <= 23872; o3++)
  r3[o3] = 0;
for (o3 = 23877; o3 <= 23884; o3++)
  r3[o3] = 0;
for (o3 = 23886; o3 <= 23894; o3++)
  r3[o3] = 0;
for (o3 = 23946; o3 <= 23948; o3++)
  r3[o3] = 0;
for (o3 = 24311; o3 <= 24313; o3++)
  r3[o3] = 0;
for (o3 = 24342; o3 <= 24347; o3++)
  r3[o3] = 0;
for (o3 = 24370; o3 <= 24374; o3++)
  r3[o3] = 10;
for (o3 = 24375; o3 <= 24381; o3++)
  r3[o3] = 0;
for (o3 = 24718; o3 <= 24721; o3++)
  r3[o3] = 0;
for (o3 = 24817; o3 <= 24821; o3++)
  r3[o3] = 0;
for (o3 = 24877; o3 <= 24882; o3++)
  r3[o3] = 0;
for (o3 = 24891; o3 <= 24893; o3++)
  r3[o3] = 0;
for (o3 = 25391; o3 <= 25395; o3++)
  r3[o3] = 0;
for (o3 = 25828; o3 <= 25838; o3++)
  r3[o3] = 0;
for (o3 = 26191; o3 <= 26195; o3++)
  r3[o3] = 0;
for (o3 = 26391; o3 <= 26393; o3++)
  r3[o3] = 0;
for (o3 = 26701; o3 <= 26722; o3++)
  r3[o3] = 0;
for (o3 = 26729; o3 <= 26799; o3++)
  r3[o3] = 2;
for (o3 = 26801; o3 <= 26803; o3++)
  r3[o3] = 2;
for (o3 = 26811; o3 <= 26813; o3++)
  r3[o3] = 2;
for (o3 = 26847; o3 <= 26870; o3++)
  r3[o3] = 2;
for (o3 = 26891; o3 <= 26899; o3++)
  r3[o3] = 0;
for (o3 = 26901; o3 <= 26923; o3++)
  r3[o3] = 0;
for (o3 = 26929; o3 <= 26946; o3++)
  r3[o3] = 0;
for (o3 = 26948; o3 <= 26998; o3++)
  r3[o3] = 0;
for (o3 = 27037; o3 <= 27040; o3++)
  r3[o3] = 0;
for (o3 = 27205; o3 <= 27232; o3++)
  r3[o3] = 0;
for (o3 = 27258; o3 <= 27260; o3++)
  r3[o3] = 0;
for (o3 = 27391; o3 <= 27398; o3++)
  r3[o3] = 0;
for (o3 = 27561; o3 <= 27564; o3++)
  r3[o3] = 0;
for (o3 = 27571; o3 <= 27574; o3++)
  r3[o3] = 0;
for (o3 = 27581; o3 <= 27584; o3++)
  r3[o3] = 0;
for (o3 = 27591; o3 <= 27594; o3++)
  r3[o3] = 0;
for (o3 = 28191; o3 <= 28193; o3++)
  r3[o3] = 0;
for (o3 = 28348; o3 <= 28358; o3++)
  r3[o3] = 0;
for (o3 = 28402; o3 <= 28432; o3++)
  r3[o3] = 0;
for (o3 = 28462; o3 <= 28492; o3++)
  r3[o3] = 0;
for (o3 = 29118; o3 <= 29122; o3++)
  r3[o3] = 0;
for (o3 = 29168; o3 <= 29172; o3++)
  r3[o3] = 0;
for (o3 = 29177; o3 <= 29185; o3++)
  r3[o3] = 0;
for (o3 = 29187; o3 <= 29195; o3++)
  r3[o3] = 0;
for (o3 = 29900; o3 <= 29903; o3++)
  r3[o3] = 0;
for (o3 = 30161; o3 <= 30179; o3++)
  r3[o3] = 0;
for (o3 = 30491; o3 <= 30494; o3++)
  r3[o3] = 0;
for (o3 = 30729; o3 <= 30732; o3++)
  r3[o3] = 0;
for (o3 = 31251; o3 <= 31259; o3++)
  r3[o3] = 0;
for (o3 = 31265; o3 <= 31268; o3++)
  r3[o3] = 0;
for (o3 = 31275; o3 <= 31279; o3++)
  r3[o3] = 0;
for (o3 = 31281; o3 <= 31297; o3++)
  r3[o3] = 0;
for (o3 = 31461; o3 <= 31469; o3++)
  r3[o3] = 0;
for (o3 = 31491; o3 <= 31495; o3++)
  r3[o3] = 0;
for (o3 = 31917; o3 <= 31922; o3++)
  r3[o3] = 0;
for (o3 = 31965; o3 <= 32e3; o3++)
  r3[o3] = 0;
for (o3 = 32001; o3 <= 32003; o3++)
  r3[o3] = 2;
for (o3 = 32005; o3 <= 32031; o3++)
  r3[o3] = 2;
for (o3 = 32033; o3 <= 32060; o3++)
  r3[o3] = 2;
for (o3 = 32064; o3 <= 32067; o3++)
  r3[o3] = 2;
for (o3 = 32074; o3 <= 32077; o3++)
  r3[o3] = 2;
for (o3 = 32081; o3 <= 32086; o3++)
  r3[o3] = 0;
for (o3 = 32107; o3 <= 32130; o3++)
  r3[o3] = 0;
for (o3 = 32133; o3 <= 32159; o3++)
  r3[o3] = 0;
for (o3 = 32164; o3 <= 32167; o3++)
  r3[o3] = 2;
for (o3 = 32180; o3 <= 32199; o3++)
  r3[o3] = 0;
for (o3 = 32201; o3 <= 32260; o3++)
  r3[o3] = 0;
for (o3 = 32301; o3 <= 32360; o3++)
  r3[o3] = 0;
for (o3 = 32401; o3 <= 32460; o3++)
  r3[o3] = 0;
for (o3 = 32501; o3 <= 32560; o3++)
  r3[o3] = 0;
for (o3 = 32601; o3 <= 32662; o3++)
  r3[o3] = 0;
for (o3 = 32664; o3 <= 32667; o3++)
  r3[o3] = 2;
for (o3 = 32701; o3 <= 32761; o3++)
  r3[o3] = 0;
for (o3 = 53001; o3 <= 53004; o3++)
  r3[o3] = 0;
for (o3 = 53008; o3 <= 53019; o3++)
  r3[o3] = 0;
for (o3 = 53021; o3 <= 53032; o3++)
  r3[o3] = 0;
for (o3 = 53034; o3 <= 53037; o3++)
  r3[o3] = 0;
for (o3 = 53042; o3 <= 53046; o3++)
  r3[o3] = 0;
for (o3 = 53074; o3 <= 53080; o3++)
  r3[o3] = 0;
for (o3 = 54001; o3 <= 54004; o3++)
  r3[o3] = 0;
for (o3 = 54008; o3 <= 54019; o3++)
  r3[o3] = 0;
for (o3 = 54021; o3 <= 54032; o3++)
  r3[o3] = 0;
for (o3 = 54034; o3 <= 54037; o3++)
  r3[o3] = 0;
for (o3 = 54042; o3 <= 54046; o3++)
  r3[o3] = 0;
for (o3 = 54048; o3 <= 54053; o3++)
  r3[o3] = 0;
for (o3 = 54074; o3 <= 54080; o3++)
  r3[o3] = 0;
for (o3 = 54098; o3 <= 54101; o3++)
  r3[o3] = 0;
for (o3 = 102001; o3 <= 102040; o3++)
  r3[o3] = 0;
for (o3 = 102042; o3 <= 102063; o3++)
  r3[o3] = 0;
for (o3 = 102065; o3 <= 102067; o3++)
  r3[o3] = 0;
for (o3 = 102070; o3 <= 102117; o3++)
  r3[o3] = 0;
for (o3 = 102122; o3 <= 102216; o3++)
  r3[o3] = 0;
for (o3 = 102221; o3 <= 102377; o3++)
  r3[o3] = 0;
for (o3 = 102382; o3 <= 102388; o3++)
  r3[o3] = 0;
for (o3 = 102389; o3 <= 102398; o3++)
  r3[o3] = 2;
for (o3 = 102399; o3 <= 102444; o3++)
  r3[o3] = 0;
for (o3 = 102445; o3 <= 102447; o3++)
  r3[o3] = 2;
for (o3 = 102448; o3 <= 102458; o3++)
  r3[o3] = 0;
for (o3 = 102459; o3 <= 102468; o3++)
  r3[o3] = 2;
for (o3 = 102469; o3 <= 102499; o3++)
  r3[o3] = 0;
for (o3 = 102500; o3 <= 102519; o3++)
  r3[o3] = 1;
for (o3 = 102520; o3 <= 102524; o3++)
  r3[o3] = 0;
for (o3 = 102525; o3 <= 102529; o3++)
  r3[o3] = 2;
for (o3 = 102530; o3 <= 102588; o3++)
  r3[o3] = 0;
for (o3 = 102590; o3 <= 102598; o3++)
  r3[o3] = 0;
for (o3 = 102601; o3 <= 102603; o3++)
  r3[o3] = 0;
for (o3 = 102605; o3 <= 102628; o3++)
  r3[o3] = 0;
for (o3 = 102629; o3 <= 102646; o3++)
  r3[o3] = 2;
for (o3 = 102648; o3 <= 102700; o3++)
  r3[o3] = 2;
for (o3 = 102701; o3 <= 102703; o3++)
  r3[o3] = 0;
for (o3 = 102707; o3 <= 102730; o3++)
  r3[o3] = 2;
for (o3 = 102733; o3 <= 102758; o3++)
  r3[o3] = 2;
for (o3 = 102767; o3 <= 102900; o3++)
  r3[o3] = 0;
for (o3 = 102901; o3 <= 102933; o3++)
  r3[o3] = 2;
for (o3 = 102934; o3 <= 102950; o3++)
  r3[o3] = 13;
for (o3 = 102951; o3 <= 102955; o3++)
  r3[o3] = 0;
for (o3 = 102961; o3 <= 102963; o3++)
  r3[o3] = 0;
for (o3 = 102965; o3 <= 102969; o3++)
  r3[o3] = 0;
for (o3 = 102971; o3 <= 102973; o3++)
  r3[o3] = 0;
for (o3 = 102975; o3 <= 102989; o3++)
  r3[o3] = 0;
for (o3 = 102990; o3 <= 102992; o3++)
  r3[o3] = 1;
for (o3 = 102997; o3 <= 103002; o3++)
  r3[o3] = 0;
for (o3 = 103003; o3 <= 103008; o3++)
  r3[o3] = 2;
for (o3 = 103009; o3 <= 103011; o3++)
  r3[o3] = 0;
for (o3 = 103012; o3 <= 103014; o3++)
  r3[o3] = 2;
for (o3 = 103019; o3 <= 103021; o3++)
  r3[o3] = 0;
for (o3 = 103022; o3 <= 103024; o3++)
  r3[o3] = 2;
for (o3 = 103029; o3 <= 103031; o3++)
  r3[o3] = 0;
for (o3 = 103032; o3 <= 103034; o3++)
  r3[o3] = 2;
for (o3 = 103065; o3 <= 103068; o3++)
  r3[o3] = 0;
for (o3 = 103074; o3 <= 103076; o3++)
  r3[o3] = 0;
for (o3 = 103077; o3 <= 103079; o3++)
  r3[o3] = 1;
for (o3 = 103080; o3 <= 103082; o3++)
  r3[o3] = 0;
for (o3 = 103083; o3 <= 103085; o3++)
  r3[o3] = 2;
for (o3 = 103090; o3 <= 103093; o3++)
  r3[o3] = 0;
for (o3 = 103097; o3 <= 103099; o3++)
  r3[o3] = 0;
for (o3 = 103100; o3 <= 103102; o3++)
  r3[o3] = 2;
for (o3 = 103107; o3 <= 103109; o3++)
  r3[o3] = 0;
for (o3 = 103110; o3 <= 103112; o3++)
  r3[o3] = 2;
for (o3 = 103113; o3 <= 103116; o3++)
  r3[o3] = 0;
for (o3 = 103117; o3 <= 103120; o3++)
  r3[o3] = 2;
for (o3 = 103153; o3 <= 103157; o3++)
  r3[o3] = 0;
for (o3 = 103158; o3 <= 103162; o3++)
  r3[o3] = 2;
for (o3 = 103163; o3 <= 103165; o3++)
  r3[o3] = 0;
for (o3 = 103166; o3 <= 103168; o3++)
  r3[o3] = 1;
for (o3 = 103169; o3 <= 103171; o3++)
  r3[o3] = 2;
for (o3 = 103186; o3 <= 103188; o3++)
  r3[o3] = 0;
for (o3 = 103189; o3 <= 103191; o3++)
  r3[o3] = 2;
for (o3 = 103192; o3 <= 103195; o3++)
  r3[o3] = 0;
for (o3 = 103196; o3 <= 103199; o3++)
  r3[o3] = 2;
for (o3 = 103200; o3 <= 103224; o3++)
  r3[o3] = 0;
for (o3 = 103225; o3 <= 103227; o3++)
  r3[o3] = 1;
for (o3 = 103232; o3 <= 103237; o3++)
  r3[o3] = 0;
for (o3 = 103238; o3 <= 103243; o3++)
  r3[o3] = 2;
for (o3 = 103244; o3 <= 103246; o3++)
  r3[o3] = 0;
for (o3 = 103247; o3 <= 103249; o3++)
  r3[o3] = 2;
for (o3 = 103254; o3 <= 103256; o3++)
  r3[o3] = 0;
for (o3 = 103257; o3 <= 103259; o3++)
  r3[o3] = 2;
for (o3 = 103264; o3 <= 103266; o3++)
  r3[o3] = 0;
for (o3 = 103267; o3 <= 103269; o3++)
  r3[o3] = 2;
for (o3 = 103300; o3 <= 103375; o3++)
  r3[o3] = 0;
for (o3 = 103381; o3 <= 103383; o3++)
  r3[o3] = 0;
for (o3 = 103384; o3 <= 103386; o3++)
  r3[o3] = 1;
for (o3 = 103387; o3 <= 103389; o3++)
  r3[o3] = 0;
for (o3 = 103390; o3 <= 103392; o3++)
  r3[o3] = 2;
for (o3 = 103397; o3 <= 103399; o3++)
  r3[o3] = 0;
for (o3 = 103400; o3 <= 103471; o3++)
  r3[o3] = 2;
for (o3 = 103476; o3 <= 103478; o3++)
  r3[o3] = 0;
for (o3 = 103479; o3 <= 103481; o3++)
  r3[o3] = 2;
for (o3 = 103486; o3 <= 103488; o3++)
  r3[o3] = 0;
for (o3 = 103489; o3 <= 103491; o3++)
  r3[o3] = 2;
for (o3 = 103492; o3 <= 103495; o3++)
  r3[o3] = 0;
for (o3 = 103496; o3 <= 103499; o3++)
  r3[o3] = 2;
for (o3 = 103528; o3 <= 103543; o3++)
  r3[o3] = 0;
for (o3 = 103544; o3 <= 103548; o3++)
  r3[o3] = 2;
for (o3 = 103549; o3 <= 103551; o3++)
  r3[o3] = 0;
for (o3 = 103552; o3 <= 103554; o3++)
  r3[o3] = 1;
for (o3 = 103555; o3 <= 103557; o3++)
  r3[o3] = 2;
for (o3 = 103558; o3 <= 103560; o3++)
  r3[o3] = 0;
for (o3 = 103571; o3 <= 103573; o3++)
  r3[o3] = 0;
for (o3 = 103574; o3 <= 103576; o3++)
  r3[o3] = 2;
for (o3 = 103577; o3 <= 103580; o3++)
  r3[o3] = 0;
for (o3 = 103581; o3 <= 103583; o3++)
  r3[o3] = 2;
for (o3 = 103595; o3 <= 103694; o3++)
  r3[o3] = 0;
for (o3 = 103696; o3 <= 103699; o3++)
  r3[o3] = 0;
for (o3 = 103700; o3 <= 103793; o3++)
  r3[o3] = 2;
for (o3 = 103794; o3 <= 103890; o3++)
  r3[o3] = 0;
for (o3 = 103891; o3 <= 103896; o3++)
  r3[o3] = 2;
for (o3 = 103900; o3 <= 103971; o3++)
  r3[o3] = 2;
for (o3 = 103972; o3 <= 103977; o3++)
  r3[o3] = 0;
for (o3 = 112e3; o3 <= 112101; o3++)
  r3[o3] = 0;

// node_modules/@arcgis/core/geometry/support/spatialReferenceUtils.js
var l2 = { 102113: true, 102100: true, 3857: true, 3785: true };
var E2 = { 4326: true, 3785: true, 3857: true, 102113: true, 102100: true, 104905: true, 104971: true };
var M2 = 'PROJCS["WGS_1984_Web_Mercator_Auxiliary_Sphere",GEOGCS["GCS_WGS_1984",DATUM["D_WGS_1984",SPHEROID["WGS_1984",6378137.0,298.257223563]],PRIMEM["Greenwich",0.0],UNIT["Degree",0.0174532925199433]],PROJECTION["Mercator_Auxiliary_Sphere"],PARAMETER["False_Easting",0.0],PARAMETER["False_Northing",0.0],PARAMETER["Central_Meridian",{Central_Meridian}],PARAMETER["Standard_Parallel_1",0.0],PARAMETER["Auxiliary_Sphere_Type",0.0],UNIT["Meter",1.0]]';
var S3 = [-20037508342788905e-9, 20037508342788905e-9];
var R = [-20037508342787e-6, 20037508342787e-6];
var u2 = { 102113: { wkTemplate: 'PROJCS["WGS_1984_Web_Mercator",GEOGCS["GCS_WGS_1984_Major_Auxiliary_Sphere",DATUM["D_WGS_1984_Major_Auxiliary_Sphere",SPHEROID["WGS_1984_Major_Auxiliary_Sphere",6378137.0,0.0]],PRIMEM["Greenwich",0.0],UNIT["Degree",0.0174532925199433]],PROJECTION["Mercator"],PARAMETER["False_Easting",0.0],PARAMETER["False_Northing",0.0],PARAMETER["Central_Meridian",{Central_Meridian}],PARAMETER["Standard_Parallel_1",0.0],UNIT["Meter",1.0]]', valid: S3, origin: R, dx: 1e-5 }, 102100: { wkTemplate: M2, valid: S3, origin: R, dx: 1e-5 }, 3785: { wkTemplate: 'PROJCS["WGS_1984_Web_Mercator",GEOGCS["GCS_WGS_1984_Major_Auxiliary_Sphere",DATUM["D_WGS_1984_Major_Auxiliary_Sphere",SPHEROID["WGS_1984_Major_Auxiliary_Sphere",6378137.0,0.0]],PRIMEM["Greenwich",0.0],UNIT["Degree",0.0174532925199433]],PROJECTION["Mercator"],PARAMETER["False_Easting",0.0],PARAMETER["False_Northing",0.0],PARAMETER["Central_Meridian",{Central_Meridian}],PARAMETER["Standard_Parallel_1",0.0],UNIT["Meter",1.0]]', valid: S3, origin: R, dx: 1e-5 }, 3857: { wkTemplate: M2, valid: S3, origin: R, dx: 1e-5 }, 4326: { wkTemplate: 'GEOGCS["GCS_WGS_1984",DATUM["D_WGS_1984",SPHEROID["WGS_1984",6378137.0,298.257223563]],PRIMEM["Greenwich",{Central_Meridian}],UNIT["Degree",0.0174532925199433]]', altTemplate: 'PROJCS["WGS_1984_Plate_Carree",GEOGCS["GCS_WGS_1984",DATUM["D_WGS_1984",SPHEROID["WGS_1984",6378137.0,298.257223563]],PRIMEM["Greenwich",0.0],UNIT["Degree",0.0174532925199433]],PROJECTION["Plate_Carree"],PARAMETER["False_Easting",0.0],PARAMETER["False_Northing",0.0],PARAMETER["Central_Meridian",{Central_Meridian}],UNIT["Degrees",111319.491]]', valid: [-180, 180], origin: [-180, 90], dx: 1e-5 }, 104971: { wkTemplate: 'GEOGCS["Mars_2000_(Sphere)",DATUM["Mars_2000_(Sphere)",SPHEROID["Mars_2000_(Sphere)",3396190.0,0.0]],PRIMEM["Reference_Meridian",0.0],UNIT["Degree",0.0174532925199433]]', valid: [-180, 180], origin: [-180, 90], dx: 1e-5 }, 104905: { wkTemplate: 'GEOGCS["GCS_Mars_2000",DATUM["D_Mars_2000",SPHEROID["Mars_2000_IAU_IAG",3396190.0,169.8944472236118]],PRIMEM["Reference_Meridian",0.0],UNIT["Degree",0.0174532925199433]]', valid: [-180, 180], origin: [-180, 90], dx: 1e-5 } };
function d2(e2, r4) {
  if (e2 === r4)
    return true;
  if (null == e2 || null == r4)
    return false;
  if (null != e2.wkid || null != r4.wkid)
    return e2.wkid === r4.wkid || s2(e2) && s2(r4) || null != r4.latestWkid && e2.wkid === r4.latestWkid || null != e2.latestWkid && r4.wkid === e2.latestWkid;
  const i3 = e2.wkt2 ?? e2.wkt, n2 = r4.wkt2 ?? r4.wkt;
  return !(!i3 || !n2) && i3.toUpperCase() === n2.toUpperCase();
}
function G(e2) {
  return W(e2) && e2.wkid && u2[e2.wkid] || null;
}
function T2(e2) {
  if (!e2)
    return;
  if (e2.wkid)
    return String(e2.wkid);
  const r4 = e2.wkt || e2.wkt2;
  return JSON.stringify(r4 ? { wkt: r4 } : {});
}
function A2(e2) {
  if (!W(e2))
    return false;
  if (e2.wkid)
    return null == r3[e2.wkid];
  const r4 = e2.wkt2 || e2.wkt;
  return !!r4 && /^\s*(?:GEOGCS|GEOGCRS|GEOGRAPHICCRS)/i.test(r4);
}
function P2(e2) {
  return !(c2(e2) || p2(e2));
}
function k(e2) {
  return W(e2) && 4326 === e2.wkid;
}
function w(e2) {
  return /^\s*(?:GEOD|GEODETIC|GEOG|GEOGRAPHIC|PROJ|PROJECTED|VERT|VERTICAL|ENG|ENGINEERING|PARAMETRIC|TIME|DERIVEDPROJ|COMPOUND|BOUND)CRS/i.test(e2);
}
function C(e2) {
  return W(e2) && e2.wkid === S2.CGCS2000;
}
function s2(e2) {
  return W(e2) && null != e2.wkid && true === l2[e2.wkid];
}
function O2(e2) {
  return W(e2) && 32662 === e2.wkid;
}
function I(e2) {
  return e2 === S2.GCSMARS2000 || e2 === S2.GCSMARS2000_SPHERE;
}
function c2(e2) {
  return W(e2) && null != e2.wkid && I(e2.wkid);
}
function D(e2) {
  return e2 === S2.GCSMOON2000;
}
function p2(e2) {
  return W(e2) && null != e2.wkid && D(e2.wkid);
}
function f2(e2) {
  return W(e2) && null != e2.wkid && true === E2[e2.wkid];
}
function W(e2) {
  return null != e2 && (null != e2.wkid && e2.wkid >= 2e3 || null != e2.wkt2 || null != e2.wkt);
}
var N2 = { wkid: 4326, wkt: r(u2[4326].wkTemplate, { Central_Meridian: "0.0" }) };
var g2 = { wkid: 102100, latestWkid: 3857 };
var U = { wkid: 32662 };

// node_modules/@arcgis/core/geometry/support/Ellipsoid.js
var i2 = class {
  constructor(i3, s4, t3, e2) {
    this.semiMajorAxis = i3, this.flattening = s4, this.outerAtmosphereRimWidth = t3;
    const h2 = 1 - this.flattening;
    this.semiMinorAxis = this.semiMajorAxis * h2, this.halfSemiMajorAxis = this.semiMajorAxis / 2, this.halfCircumference = Math.PI * this.semiMajorAxis, this.metersPerDegree = this.halfCircumference / 180, this.inverseFlattening = 1 / (1 - this.flattening) - 1, this.eccentricitySquared = e2 || 2 * this.flattening - this.flattening * this.flattening, this.meanRadiusSemiAxes = (2 * this.semiMajorAxis + this.semiMinorAxis) / 3;
  }
  get radius() {
    return this.semiMajorAxis;
  }
};
var s3 = new i2(6378137, 1 / 298.257223563, 3e5, 0.006694379990137799);
var t2 = new i2(3396190, 1 / 169.8944472236118, 23e4);
var e = new i2(1737400, 0, 0);

// node_modules/@arcgis/core/geometry/ellipsoidUtils.js
function T3(n2) {
  return { wkt: `GEOCCS["Spherical geocentric",
    DATUM["Not specified",
      SPHEROID["Sphere",${n2.radius},0]],
    PRIMEM["Greenwich",0.0,
      AUTHORITY["EPSG","8901"]],
    UNIT["m",1.0],
    AXIS["Geocentric X",OTHER],
    AXIS["Geocentric Y",EAST],
    AXIS["Geocentric Z",NORTH]
  ]` };
}
var G2 = T3(s3);
var I2 = T3(t2);
var E3 = T3(e);
var u3 = { wkt: `GEOCCS["WGS 84",
  DATUM["WGS_1984",
    SPHEROID["WGS 84",${s3.radius},298.257223563,
      AUTHORITY["EPSG","7030"]],
    AUTHORITY["EPSG","6326"]],
  PRIMEM["Greenwich",0,
    AUTHORITY["EPSG","8901"]],
  UNIT["m",1.0,
    AUTHORITY["EPSG","9001"]],
  AXIS["Geocentric X",OTHER],
  AXIS["Geocentric Y",OTHER],
  AXIS["Geocentric Z",NORTH],
  AUTHORITY["EPSG","4978"]
]` };
function R2(o5) {
  return null != o5 && (c2(o5) || d2(o5, I2)) ? t2 : null != o5 && (p2(o5) || d2(o5, E3)) ? e : s3;
}
function A3(t3) {
  return I(t3) ? t2 : D(t3) ? e : s3;
}
function O3(n2) {
  return d2(n2, G2);
}
function H(n2) {
  return O3(n2) || d2(n2, I2) || d2(n2, E3);
}

// node_modules/@arcgis/core/core/unitUtils.js
var o4 = 39.37;
var m = s3.radius * Math.PI / 200;
var f3 = /(?:LENGTH)?UNIT\[([^\]]+)]]$/i;
var d3 = r3;
var U2 = /UNIT\[([^\]]+)]/i;
var q = /* @__PURE__ */ new Set([4261, 4305, 4807, 4810, 4811, 4812, 4816, 4819, 4821, 4901, 4902, 37225, 104139, 104140]);
var h = o()({ meter: "meters", foot: "feet", foot_us: "us-feet", foot_clarke: "clarke-feet", yard_clarke: "clarke-yards", link_clarke: "clarke-links", yard_sears: "sears-yards", foot_sears: "sears-feet", chain_sears: "sears-chains", chain_benoit_1895_b: "benoit-1895-b-chains", yard_indian: "indian-yards", yard_indian_1937: "indian-1937-yards", foot_gold_coast: "gold-coast-feet", chain_sears_1922_truncated: "sears-1922-truncated-chains", "50_kilometers": "50-kilometers", "150_kilometers": "150-kilometers" });
var B = (e2) => e2 * e2;
var k2 = (e2) => e2 * e2 * e2;
var p3 = { length: { baseUnit: "meters", units: { millimeters: { inBaseUnits: 1e-3 }, centimeters: { inBaseUnits: 0.01 }, decimeters: { inBaseUnits: 0.1 }, meters: { inBaseUnits: 1 }, kilometers: { inBaseUnits: 1e3 }, inches: { inBaseUnits: 0.0254 }, feet: { inBaseUnits: 0.3048 }, yards: { inBaseUnits: 0.9144 }, miles: { inBaseUnits: 1609.344 }, "nautical-miles": { inBaseUnits: 1852 }, "us-feet": { inBaseUnits: 1200 / 3937 } } }, area: { baseUnit: "square-meters", units: { "square-millimeters": { inBaseUnits: B(1e-3) }, "square-centimeters": { inBaseUnits: B(0.01) }, "square-decimeters": { inBaseUnits: B(0.1) }, "square-meters": { inBaseUnits: 1 }, "square-kilometers": { inBaseUnits: B(1e3) }, "square-inches": { inBaseUnits: B(0.0254) }, "square-feet": { inBaseUnits: B(0.3048) }, "square-yards": { inBaseUnits: B(0.9144) }, "square-miles": { inBaseUnits: B(1609.344) }, "square-us-feet": { inBaseUnits: B(1200 / 3937) }, acres: { inBaseUnits: 15625e-7 * B(1609.344) }, ares: { inBaseUnits: 100 }, hectares: { inBaseUnits: 1e4 } } }, volume: { baseUnit: "liters", units: { liters: { inBaseUnits: 1 }, "cubic-millimeters": { inBaseUnits: 1e3 * k2(1e-3) }, "cubic-centimeters": { inBaseUnits: 1e3 * k2(0.01) }, "cubic-decimeters": { inBaseUnits: 1e3 * k2(0.1) }, "cubic-meters": { inBaseUnits: 1e3 }, "cubic-kilometers": { inBaseUnits: 1e3 * k2(1e3) }, "cubic-inches": { inBaseUnits: 1e3 * k2(0.0254) }, "cubic-feet": { inBaseUnits: 1e3 * k2(0.3048) }, "cubic-yards": { inBaseUnits: 1e3 * k2(0.9144) }, "cubic-miles": { inBaseUnits: 1e3 * k2(1609.344) } } }, angle: { baseUnit: "radians", units: { radians: { inBaseUnits: 1 }, degrees: { inBaseUnits: Math.PI / 180 } } } };
var b = (() => {
  const e2 = {};
  for (const s4 in p3)
    for (const t3 in p3[s4].units)
      e2[t3] = s4;
  return e2;
})();
function y2(e2, s4, t3) {
  return e2 * p3[t3].units[s4].inBaseUnits;
}
function w2(e2, s4, t3) {
  return e2 / p3[t3].units[s4].inBaseUnits;
}
function D2(e2) {
  const s4 = b[e2];
  if (!s4)
    throw new Error("unknown type");
  return s4;
}
function I3(e2, s4 = null) {
  return s4 = s4 || D2(e2), p3[s4].baseUnit === e2;
}
function j(e2, s4, t3) {
  if (s4 === t3)
    return e2;
  const r4 = D2(s4);
  if (r4 !== D2(t3))
    throw new Error("incompatible units");
  const n2 = I3(s4, r4) ? e2 : y2(e2, s4, r4);
  return I3(t3, r4) ? n2 : w2(n2, t3, r4);
}
function H2(e2, s4, t3) {
  return j(e2, s4, "meters") / (t3 * Math.PI / 180);
}
function L(e2) {
  return h.fromJSON(e2.toLowerCase()) || null;
}
function W2(e2) {
  if (null != e2 && !P2(e2))
    return 1;
  const s4 = z(e2);
  return s4 > 1e5 ? 1 : s4;
}
function $(e2) {
  return z(e2) >= R2(e2).metersPerDegree ? "meters" : ee(e2);
}
function z(e2, s4 = s3.metersPerDegree) {
  return Q(e2, true) ?? s4;
}
function Q(e2, s4 = false) {
  const t3 = (e2 == null ? void 0 : e2.wkid) ?? null, i3 = (e2 == null ? void 0 : e2.wkt2) ?? (e2 == null ? void 0 : e2.wkt) ?? null;
  let c3 = null;
  if (t3) {
    if (I(t3))
      return t2.metersPerDegree;
    if (D(t3))
      return e.metersPerDegree;
    c3 = d3.values[d3[t3]], !c3 && s4 && q.has(t3) && (c3 = m);
  } else
    i3 && (ie(i3) ? c3 = X(f3.exec(i3), c3) : ne(i3) && (c3 = X(U2.exec(i3), c3)));
  return c3;
}
function V(e2) {
  return A2(e2) ? 1 : z(e2);
}
function X(e2, s4) {
  return (e2 == null ? void 0 : e2[1]) ? Z(e2[1]) : s4;
}
function Z(e2) {
  return parseFloat(e2.split(",")[1]);
}
function ee(e2) {
  const s4 = (e2 == null ? void 0 : e2.wkid) ?? null, t3 = (e2 == null ? void 0 : e2.wkt2) ?? (e2 == null ? void 0 : e2.wkt) ?? null;
  let r4 = null;
  if (s4)
    r4 = d3.units[d3[s4]];
  else if (t3) {
    const e3 = ie(t3) ? f3 : ne(t3) ? U2 : null;
    if (e3) {
      const s5 = e3.exec(t3);
      (s5 == null ? void 0 : s5[1]) && (r4 = ue(s5[1]));
    }
  }
  return null != r4 ? L(r4) : null;
}
function ne(e2) {
  return /^GEOCCS/i.test(e2);
}
function ie(e2) {
  return /^\s*(?:PROJCS|PROJCRS|PROJECTEDCRS)/i.test(e2);
}
var ae = 1e-7;
function ue(e2) {
  const s4 = /[\\"']{1}([^\\"']+)/.exec(e2);
  let t3 = s4 == null ? void 0 : s4[1];
  if (!t3 || !d3.units.includes(t3)) {
    const s5 = Z(e2);
    t3 = null;
    const r4 = d3.values;
    for (let e3 = 0; e3 < r4.length; ++e3)
      if (Math.abs(s5 - r4[e3]) < ae) {
        t3 = d3.units[e3];
        break;
      }
  }
  return t3;
}
var le = { esriAcres: "acres", esriAres: "ares", esriHectares: "hectares", esriSquareCentimeters: "square-centimeters", esriSquareDecimeters: "square-decimeters", esriSquareFeet: "square-feet", esriSquareInches: "square-inches", esriSquareKilometers: "square-kilometers", esriSquareMeters: "square-meters", esriSquareMiles: "square-miles", esriSquareMillimeters: "square-millimeters", esriSquareUsFeet: "square-us-feet", esriSquareYards: "square-yards" };
var oe = { esriCentimeters: "centimeters", esriDecimeters: "decimeters", esriFeet: "feet", esriInches: "inches", esriKilometers: "kilometers", esriMeters: "meters", esriMiles: "miles", esriMillimeters: "millimeters", esriNauticalMiles: "nautical-miles", esriYards: "yards" };
var me = { esriDUDecimalDegrees: "degrees", esriDURadians: "radians" };
var fe = o()(le);
var de = o()(oe);
var Ue = o()(me);

export {
  s3 as s,
  t2 as t,
  e,
  r2 as r,
  _,
  P,
  s as s2,
  E,
  a2 as a,
  S,
  d,
  S2,
  r3 as r2,
  d2,
  G,
  T2 as T,
  A2 as A,
  k,
  w,
  C,
  s2 as s3,
  O2 as O,
  c2 as c,
  p2 as p,
  f2 as f,
  W,
  N2 as N,
  g2 as g,
  U,
  G2,
  I2 as I,
  E3 as E2,
  u3 as u,
  A3 as A2,
  O3 as O2,
  H,
  o4 as o,
  D2 as D,
  j,
  H2,
  L,
  W2,
  $,
  z,
  V,
  ee,
  fe,
  de,
  Ue
};
//# sourceMappingURL=chunk-HLGIRLYQ.js.map
