...
function f2(i2, n) {
  if (null == i2)
    return null;
  const p2 = i2.spatialReference, a4 = G(p2), f3 = a(i2) ? i2.toJSON() : i2;
  if (!a4)
    return f3;
  const I2 = s(p2) ? 102100 : 4326, g3 = r[I2].maxX, v2 = r[I2].minX;
  if (l(f3))
    return y(f3, g3, v2);
  if (u2(f3))
    return f3.points = f3.points.map((t) => y(t, g3, v2)), f3;
  if (m(f3))
    return d(f3, a4);
  if (f(f3) || s2(f3)) {
    const t = a2(S, f3), i3 = { xmin: t[0], ymin: t[1], xmax: t[2], ymax: t[3] }, e = i(i3.xmin, v2) * (2 * g3), h = 0 === e ? f3 : s3(f3, e);
    return i3.xmin += e, i3.xmax += e, i3.xmax > g3 ? P(h, g3, n) : i3.xmin < v2 ? P(h, v2, n) : h;
  }
  return f3;
}
function d(t, i2) {
  ...
  return s4.length < 2 ? s4[0] || t : s4.length > 2 ? (t.xmin = i2.valid[0], t.xmax = i2.valid[1], t) : { rings: s4.map((t2) => [[t2.xmin, t2.ymin], [t2.xmin, t2.ymax], [t2.xmax, t2.ymax], [t2.xmax, t2.ymin], [t2.xmin, t2.ymin]]) };
}
function y(t, i2, s4) {
  if (Array.isArray(t)) {
    const n = t[0];
...
function I(t, i2) {
  const s4 = [], { ymin: n, ymax: e, xmin: h, xmax: o } = t, r2 = t.xmax - t.xmin, [u3, m2] = i2.valid, { x, frameId: _ } = g2(t.xmin, i2), { x: l2, frameId: c } = g2(t.xmax, i2), p2 = x === l2 && r2 > 0;
  if (r2 > 2 * m2) {
    const t2 = { xmin: h < o ? x : l2, ymin: n, xmax: m2, ymax: e }, i3 = { xmin: u3, ymin: n, xmax: h < o ? l2 : x, ymax: e }, r3 = { xmin: 0, ymin: n, xmax: m2, ymax: e }, p3 = { xmin: u3, ymin: n, xmax: 0, ymax: e }, a4 = [], f3 = [];
    v(t2, r3) && a4.push(_), v(t2, p3) && f3.push(_), v(i3, r3) && a4.push(c), v(i3, p3) && f3.push(c);
    for (let s5 = _ + 1; s5 < c; s5++)
      a4.push(s5), f3.push(s5);
    s4.push(new C(t2, [_]), new C(i3, [c]), new C(r3, a4), new C(p3, f3));
  } else
    x > l2 || p2 ? s4.push(new C({ xmin: x, ymin: n, xmax: m2, ymax: e }, [_]), new C({ xmin: u3, ymin: n, xmax: l2, ymax: e }, [c])) : s4.push(new C({ xmin: x, ymin: n, xmax: l2, ymax: e }, [_]));
  return s4;
}
function g2(t, i2) {
  const [s4, n] = i2.valid, e = 2 * n;
  let h, o = 0;
  return t > n ? (h = Math.ceil(Math.abs(t - n) / e), t -= h * e, o = h) : t < s4 && (h = Math.ceil(Math.abs(t - s4) / e), t += h * e, o = -h), { x: t, frameId: o };
}
function v(t, i2) {
  const { xmin: s4, ymin: n, xmax: e, ymax: h } = i2;
  return O(t, s4, n) && O(t, s4, h) && O(t, e, h) && O(t, e, n);
}
function O(t, i2, s4) {
  return i2 >= t.xmin && i2 <= t.xmax && s4 >= t.ymin && s4 <= t.ymax;
}
function P(t, i2, s4 = true) {
  const e = !s2(t);
  if (e && g(t), s4) {
...
var T = class {
  ...
  cut(t, i2) {
    ...
    if (this._xCut = i2, t.rings)
      this._closed = true, s4 = t.rings, this._minPts = 4;
    else {
      if (!t.paths)
        return null;
    ...
    for (const e of this._linesIn)
      e && e.length >= this._minPts && s4.push(e);
...
//# sourceMappingURL=chunk-L7TO2DFS.js.map
