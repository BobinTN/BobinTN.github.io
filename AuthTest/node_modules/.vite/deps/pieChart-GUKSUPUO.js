  ...
  _updateChildren() {
    if (super._updateChildren(), this.isDirty("baseRadius") || this.isPrivateDirty("radius") || this.isPrivateDirty("innerRadius") || this.isDirty("labelAngle") || this.isDirty("radius") || this.isDirty("inside") || this.isDirty("orientation") || this.isDirty("textType")) {
      ...
      if (this._display.angle = 0, "circular" == e)
        ...
        0 == l3 && (s2 = 0, n = 0);
        let t3 = n * r, o = n * a;
        "radial" == e ? (this.setRaw("x", t3), this.setRaw("y", o), s2 < 90 || s2 > 270 || "auto" != i ? (this._display.angle = s2, this._flipped = false) : (this._display.angle = s2 + 180, this._flipped = true), this._dirty.rotation = false) : "adjusted" == e ? (this.setRaw("centerX", X), this.setRaw("centerY", X), this.setRaw("x", t3), this.setRaw("y", o)) : "regular" == e && (this.setRaw("x", t3), this.setRaw("y", o));
      }
  ...
  _updatePosition() {
    ...
    if ("radial" == e) {
      if (this._flipped) {
        ...
        e2 instanceof K && (l3 *= 1 - 2 * e2.value), i = l3 * de(a), s2 = l3 * le(a);
      }
    } else
      t2 || "adjusted" != e || (i = l3 / 2 * de(a), s2 = n / 2 * le(a));
...
function W(e) {
  return e >= 1 ? Y2 : e <= -1 ? -Y2 : Math.asin(e);
}
function U2(e) {
...
function Z() {
  var e = U2, s2 = K2, a = k(0), r = null, l3 = q, n = E, o = J, h = null, c = V2(u);
  function u() {
    var t2, i, u2 = +e.apply(this, arguments), d = +s2.apply(this, arguments), g = l3.apply(this, arguments) - Y2, p = n.apply(this, arguments) - Y2, y = j(p - g), b = p > g;
    if (h || (h = t2 = c()), d < u2 && (i = d, d = u2, u2 = i), d > X2)
      if (y > V3 - X2)
        h.moveTo(d * O(g), d * B2(g)), h.arc(0, 0, d, g, p, !b), u2 > X2 && (h.moveTo(u2 * O(p), u2 * B2(p)), h.arc(0, 0, u2, p, g, b));
      else {
        var m2, f, _ = g, x = p, v = g, w = p, k2 = y, D2 = y, R = o.apply(this, arguments) / 2, A = R > X2 && (r ? +r.apply(this, arguments) : F(u2 * u2 + d * d)), P = M(j(d - u2) / 2, +a.apply(this, arguments)), T = P, L = P;
        if (A > X2) {
          var N2 = W(A / u2 * B2(R)), C2 = W(A / d * B2(R));
          (k2 -= 2 * N2) > X2 ? (v += N2 *= b ? 1 : -1, w -= N2) : (k2 = 0, v = w = (g + p) / 2), (D2 -= 2 * C2) > X2 ? (_ += C2 *= b ? 1 : -1, x -= C2) : (D2 = 0, _ = x = (g + p) / 2);
        }
        var I2 = d * O(_), U3 = d * B2(_), K3 = u2 * O(w), q2 = u2 * B2(w);
        if (P > X2) {
          var E2, J2 = d * O(x), Z2 = d * B2(x), $2 = u2 * O(v), ee2 = u2 * B2(v);
          if (y < H)
            if (E2 = G(I2, U3, $2, ee2, J2, Z2, K3, q2)) {
              var te2 = I2 - E2[0], ie2 = U3 - E2[1], se2 = J2 - E2[0], ae2 = Z2 - E2[1], re2 = 1 / B2(z2((te2 * se2 + ie2 * ae2) / (F(te2 * te2 + ie2 * ie2) * F(se2 * se2 + ae2 * ae2))) / 2), le2 = F(E2[0] * E2[0] + E2[1] * E2[1]);
              T = M(P, (u2 - le2) / (re2 - 1)), L = M(P, (d - le2) / (re2 + 1));
            } else
              T = L = 0;
        }
        D2 > X2 ? L > X2 ? (m2 = Q($2, ee2, I2, U3, d, L, b), f = Q(J2, Z2, K3, q2, d, L, b), h.moveTo(m2.cx + m2.x01, m2.cy + m2.y01), L < P ? h.arc(m2.cx, m2.cy, L, S(m2.y01, m2.x01), S(f.y01, f.x01), !b) : (h.arc(m2.cx, m2.cy, L, S(m2.y01, m2.x01), S(m2.y11, m2.x11), !b), h.arc(0, 0, d, S(m2.cy + m2.y11, m2.cx + m2.x11), S(f.cy + f.y11, f.cx + f.x11), !b), h.arc(f.cx, f.cy, L, S(f.y11, f.x11), S(f.y01, f.x01), !b))) : (h.moveTo(I2, U3), h.arc(0, 0, d, _, x, !b)) : h.moveTo(I2, U3), u2 > X2 && k2 > X2 ? T > X2 ? (m2 = Q(K3, q2, J2, Z2, u2, -T, b), f = Q(I2, U3, $2, ee2, u2, -T, b), h.lineTo(m2.cx + m2.x01, m2.cy + m2.y01), T < P ? h.arc(m2.cx, m2.cy, T, S(m2.y01, m2.x01), S(f.y01, f.x01), !b) : (h.arc(m2.cx, m2.cy, T, S(m2.y01, m2.x01), S(m2.y11, m2.x11), !b), h.arc(0, 0, u2, S(m2.cy + m2.y11, m2.cx + m2.x11), S(f.cy + f.y11, f.cx + f.x11), b), h.arc(f.cx, f.cy, T, S(f.y11, f.x11), S(f.y01, f.x01), !b))) : h.arc(0, 0, u2, w, v, b) : h.lineTo(K3, q2);
      }
    else
      h.moveTo(0, 0);
    if (h.closePath(), t2)
...
Object.defineProperty(te, "className", { enumerable: true, configurable: true, writable: true, value: "PercentChart" }), Object.defineProperty(te, "classNames", { enumerable: true, configurable: true, writable: true, value: U.classNames.concat([te.className]) });
...
var se = class extends te {
  ...
  _afterNew() {
    super._afterNew(), this.seriesContainer.setAll({ x: X, y: X });
  }
  _prepareChildren() {
    ...
    let l3 = pe(0, 0, s2, a, 1);
    const n = t2 / (l3.right - l3.left), o = i / (l3.bottom - l3.top);
    let c = { left: 0, right: 0, top: 0, bottom: 0 };
    if (r instanceof K) {
      let e2 = r.value, l4 = Math.min(n, o);
      e2 = Math.max(l4 * e2, l4 - Math.min(i, t2)) / l4, c = pe(0, 0, s2, a, e2), this.setPrivateRaw("irModifyer", e2 / r.value);
    }
    l3 = ge([l3, c]);
...
var ae = class extends ie {
  _makeSlices() {
    return new V(Be.new({}), () => $._new(this._root, { themeTags: zt(this.slices.template.get("themeTags", []), ["pie", "series"]) }, [this.slices.template]));
  }
  _makeLabels() {
    return new V(Be.new({}), () => C._new(this._root, { themeTags: zt(this.labels.template.get("themeTags", []), ["pie", "series"]) }, [this.labels.template]));
  }
  _makeTicks() {
    return new V(Be.new({}), () => B._new(this._root, { themeTags: zt(this.ticks.template.get("themeTags", []), ["pie", "series"]) }, [this.ticks.template]));
  }
  processDataItem(e) {
  ...
  _updateTick(e) {
    ...
    if (t2 && i && s2) {
      const e2 = (s2.get("shiftRadius", 0) + s2.get("radius", 0)) * s2.get("scale", 1) * a, r = i.get("labelAngle", 0), l3 = de(r), n = le(r), o = this.labelsContainer, h = o.get("paddingLeft", 0), c = o.get("paddingRight", 0);
      let g = 0, p = 0;
...
//# sourceMappingURL=pieChart-GUKSUPUO.js.map
