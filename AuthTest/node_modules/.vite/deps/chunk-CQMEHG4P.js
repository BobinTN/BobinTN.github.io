...
var O = w = class extends f {
  static create(e2 = {}) {
    const { resolutionFactor: t2 = 1, scales: o2, size: s2 = 256, spatialReference: i2 = f2.WebMercator, numLODs: l2 = 24 } = e2;
    if (!W(i2)) {
      const e3 = [];
      if (o2)
      ...
      return new w({ dpi: 96, lods: e3, origin: new x(0, 0, i2), size: [s2, s2], spatialReference: i2 });
    }
    const n = G(i2), a = e2.origin ? new x({ x: e2.origin.x, y: e2.origin.y, spatialReference: i2 }) : new x(n ? { x: n.origin[0], y: n.origin[1], spatialReference: i2 } : { x: 0, y: 0, spatialReference: i2 }), u2 = 96, d2 = 1 / (z(i2) * 39.37 * u2), g2 = [];
    if (o2)
      ...
      let e3 = A(i2) ? 512 / s2 * 5916575275917094e-7 : 256 / s2 * 591657527591555e-6;
      const o3 = Math.ceil(l2 / t2);
      g2.push(new p({ level: 0, scale: e3, resolution: e3 * d2 }));
      for (let r2 = 1; r2 < o3; r2++) {
        const o4 = e3 / 2 ** t2, s3 = o4 * d2;
        g2.push(new p({ level: r2, scale: o4, resolution: s3 })), e3 = o4;
      }
    }
    return new w({ dpi: u2, lods: g2, origin: a, size: [s2, s2], spatialReference: i2 });
  }
  constructor(e2) {
  ...
  tileAt(e2, t2, o2, r2) {
    const s2 = this.lodAt(e2);
    if (!s2)
      return null;
    let i2, l2;
    if ("number" == typeof t2)
      i2 = t2, l2 = o2;
    else if (d(t2.spatialReference, this.spatialReference))
      i2 = t2.x, l2 = t2.y, r2 = o2;
    else {
      const e3 = g(t2, this.spatialReference);
      if (null == e3)
        return null;
      i2 = e3.x, l2 = e3.y, r2 = o2;
    }
    const n = s2.resolution * this.size[0], a = s2.resolution * this.size[1];
    return r2 || (r2 = new t(null, 0, 0, 0, u())), r2.level = e2, r2.row = Math.floor((this.origin.y - l2) / a + 1e-3), r2.col = Math.floor((i2 - this.origin.x) / n + 1e-3), this.updateTileInfo(r2), r2;
  }
...
//# sourceMappingURL=chunk-CQMEHG4P.js.map
