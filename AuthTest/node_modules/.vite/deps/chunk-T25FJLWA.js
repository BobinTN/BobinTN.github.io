...
function r(r3, t2, a3) {
  ...
  t2.contextTimeZone = (_a = a3.$view) == null ? void 0 : _a.timeZone;
  try {
    return r3.evaluate({ ...a3, $feature: n2 }, r3.services);
  } catch (u2) {
    return s.getLogger("esri.views.2d.support.arcadeOnDemand").warn("Feature arcade evaluation failed:", u2), null;
  }
}
...
var a2 = class {
    ...
    for (const e in r3) {
      ...
      switch (s3.type) {
        case "field":
          break;
        case "expression":
          o3.push(this._createArcadeComputedField(s3));
          break;
        case "label-expression":
          o3.push(this._createLabelArcadeComputedField(s3));
          break;
        case "statistic":
          c2.push(s3);
      }
    }
    this._computedFields = await Promise.all(o3), this._canCacheExpressionValue = !this._computedFields.some((e) => "expression" === e.type && null != e.expression && e.expression.referencesScale()), this._statisticFields = c2;
  }
  setComputedAttributes(e, s3, t2, i3, a3) {
    const r3 = this._bitsets.computed;
    if (!this._canCacheExpressionValue || !r3.has(t2)) {
      r3.set(t2);
      for (const r4 of this._computedFields) {
        const o3 = this._evaluateField(s3, r4, i3, a3);
        switch (r4.resultType) {
          case "numeric":
            e.setComputedNumericAtIndex(t2, r4.fieldIndex, o3);
            break;
          case "string":
            e.setComputedStringAtIndex(t2, r4.fieldIndex, o3);
        }
      }
    }
  }
  async _createArcadeComputedField(e) {
    ...
    return { ...e, expression: await n(e.valueExpression, s3, i3) };
  }
  async _createLabelArcadeComputedField(e) {
    const s3 = this._sourceInfo.spatialReference, t2 = this._sourceInfo.fieldsIndex, { createLabelFunction: i3 } = await import("./labelFormatUtils-UH7AQ6Z5.js"), a3 = await i3(e.label, t2, s3);
    return { ...e, builder: a3 };
  }
  _evaluateField(e, s3, t2, a3) {
    switch (s3.type) {
      case "label-expression": {
        const t3 = e.readArcadeFeature();
        return s3.builder.evaluate(t3, a3) || "";
      }
      case "expression": {
        const { expression: r3 } = s3;
        return r(r3, e, { $view: { scale: t2, timeZone: a3 } });
      }
    }
  }
};
...
var r2 = class _r extends S {
  ...
  constructor(r3, t2) {
    super(S.createInstance(), r3.fullSchema()), this._currentIndex = -1, this._reader = r3, this._indices = t2;
  }
  get fields() {
...
var I = { getObjectId: (t2) => t2.getObjectId(), getAttributes: (t2) => t2.readAttributes(), getAttribute: (t2, e) => t2.readAttribute(e), getAttributeAsTimestamp: (t2, e) => t2.readAttributeAsTimestamp(e), cloneWithGeometry: (t2, e) => t2, getGeometry: (t2) => t2.readHydratedGeometry(), getCentroid: (t2, e) => t2.readCentroid() };
var u = class extends a2 {
  constructor(a3, r3, n2) {
    super(a3, r3), this.featureAdapter = I, this.events = new o(), this._featureSetsByInstance = /* @__PURE__ */ new Map(), this._objectIdToDisplayId = /* @__PURE__ */ new Map(), this._spatialIndexInvalid = true, this._indexSearchCache = new t(50), this._index = i2(9, (t2) => ({ minX: this._storage.getXMin(t2), minY: this._storage.getYMin(t2), maxX: this._storage.getXMax(t2), maxY: this._storage.getYMax(t2) })), this.mode = n2;
  }
  get storeStatistics() {
    let t2 = 0, e = 0, s3 = 0;
  ...
  onTileData(t2, e, s3) {
    ...
    if (e.addOrUpdate.attachStorage(this._storage), "snapshot" === this.mode) {
      const a4 = e.addOrUpdate.getCursor();
      for (; a4.next(); ) {
        const e2 = a4.getDisplayId();
        this.setComputedAttributes(this._storage, a4, e2, t2.scale, s3);
      }
      return e;
    }
    ...
    for (; a3.next(); )
      this._insertFeature(a3, t2.scale, s3);
  ...
  search(t2) {
    ...
    if (null != s3)
      return s3.readers;
    const a3 = /* @__PURE__ */ new Map(), r3 = this._searchIndex(t2.bounds), n2 = [];
    for (const i3 of r3) {
  ...
  remove(t2) {
    ...
    if (!e)
      return;
    const s3 = this._storage.getInstanceId(e), a3 = c(s3), r3 = h(s3), n2 = this._featureSetsByInstance.get(r3);
    this._objectIdToDisplayId.delete(t2), this._storage.releaseDisplayId(e), n2.removeAtIndex(a3), n2.isEmpty && this._featureSetsByInstance.delete(r3), this._spatialIndexInvalid = true;
  }
  forEach(t2) {
  ...
  forEachUnsafe(t2) {
    this._objectIdToDisplayId.forEach((e) => {
      const s3 = this._storage.getInstanceId(e), a3 = h(s3), r3 = c(s3), n2 = this._getFeatureSet(a3);
  ...
  forEachBounds(t2, e) {
    ...
    for (const s3 of t2) {
      ...
      d(o2, this._storage.getXMin(t3), this._storage.getYMin(t3), this._storage.getXMax(t3), this._storage.getYMax(t3)), e(o2);
    }
  }
  ...
  _insertFeature(t2, e, s3) {
    const a3 = this._storage, r3 = t2.getObjectId(), n2 = d2(t2.instance, t2.getIndex());
    a3.getInstanceId(t2.getDisplayId());
    let i3 = this._objectIdToDisplayId.get(r3);
    i3 || (i3 = a3.createDisplayId(), this._objectIdToDisplayId.set(r3, i3), this._spatialIndexInvalid = true), t2.setDisplayId(i3), a3.setInstanceId(i3, n2), this.setComputedAttributes(a3, t2, i3, e, s3);
  }
  _searchIndex(t2) {
    this._rebuildIndex();
...
//# sourceMappingURL=chunk-T25FJLWA.js.map
