...
var t = class {
  ...
  prune(e2, t2, i2) {
    ...
    for (; r2 > this.maxSize && s >= 0; )
      s !== e2 && (r2 = this._pruneAroundCenterTile(r2, t2, i2, s)), s--;
    r2 > this.maxSize && (r2 = this._pruneAroundCenterTile(r2, t2, i2, e2));
  }
  _pruneAroundCenterTile(t2, i2, r2, s) {
    const l2 = this._tileKeysPerLevel[s];
    if (!l2 || 0 === l2.length)
      return t2;
    const { size: n, origin: h2 } = this._tileInfoView.tileInfo, o2 = r2 * n[0], _ = r2 * n[1], d = [0, 0], u2 = [0, 0];
    for (l2.sort((t3, r3) => (d[0] = h2.x + o2 * (t3.col + 0.5), d[1] = h2.y - _ * (t3.row + 0.5), u2[0] = h2.x + o2 * (r3.col + 0.5), u2[1] = h2.y - _ * (r3.row + 0.5), b(d, i2) - b(u2, i2))); l2.length > 0; ) {
      const e2 = l2.pop();
...
var r = class {
  ...
  update(e2) {
    var _a, _b;
    const { resampling: i2, tileIndex: t2 } = this, { scale: l2, center: r2, resolution: n } = e2.state, { minScale: f, maxScale: d } = this.tileInfoView, u2 = !e2.stationary && l2 > this._previousScale;
    ...
    if (y.length > 0)
      for (const { row: s, colFrom: a2, colTo: c2 } of y)
        for (let e3 = a2; e3 <= c2; e3++) {
          v++;
          const i3 = o.set(I, s, g.normalizeCol(e3), g.getWorldForColumn(e3)).id;
          let l3 = t2.get(i3);
          if (l3)
            l3.isReady ? (h.set(i3, l3), m++) : u2 || this._addParentTile(i3, h);
          else {
    ...
    for (const [s, _] of t2) {
      if (h.has(s))
        continue;
      o.set(s);
      const e3 = this.tileInfoView.intersects(p, o), i3 = "purge" === this.cachePolicy ? o.level !== I : o.level > I;
      !e3 || !u2 && T ? !i3 && e3 || a.push(_) : _.isReady ? i3 && "purge" === this.cachePolicy && this._hasReadyAncestor(o, I) ? a.push(_) : c.push(_) : i3 && a.push(_);
    }
    for (const s of c)
  ...
  refresh(e2) {
    ...
    for (const i2 of this.tileIndex.values())
      this.tiles.includes(i2) ? e2(i2) : a.push(i2);
    for (const i2 of a)
...
//# sourceMappingURL=chunk-DZE6ZO7Z.js.map
