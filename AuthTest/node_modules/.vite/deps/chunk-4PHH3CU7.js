...
var w = s.getLogger("esri.layers.graphics.sources.ogcfeature");
var b = "http://www.opengis.net/def/crs/";
var h = `${b}OGC/1.3/CRS84`;
async function j2(n, i3, r = {}, a = 5) {
  const { links: o2 } = n, s3 = G(o2, "items", "application/geo+json") || G(o2, "http://www.opengis.net/def/rel/ogc/1.0/items", "application/geo+json");
  if (null == s3)
    throw new s2("ogc-feature-layer:missing-items-page", "Missing items url");
  const { data: l } = await j(s3.href, { signal: r.signal, query: { limit: a, ...r.customParameters, token: r.apiKey }, headers: { accept: "application/geo+json" } });
  E(l);
  const d = I(l, { geometryType: i3.geometryType }), y = i3.fields || d.fields || [], b2 = null != i3.hasZ ? i3.hasZ : d.hasZ, h2 = d.geometryType, j3 = i3.objectIdField || d.objectIdFieldName || "OBJECTID";
  let F2 = i3.timeInfo;
  const I3 = y.find(({ name: e2 }) => e2 === j3);
  if (I3)
    ...
    y.unshift({ name: j3, alias: j3, type: "number" === d.objectIdFieldType ? "esriFieldTypeOID" : "esriFieldTypeString", editable: false, nullable: false });
  }
  if (j3 !== d.objectIdFieldName) {
  ...
  if (F2) {
    ...
    F2.timeReference || (F2.timeReference = { timeZoneIANA: i }), F2.startTimeField || F2.endTimeField || (w.warn({ name: "ogc-feature-layer:invalid-timeInfo", message: "startTimeField and endTimeField are missing", details: { timeInfo: F2 } }), F2 = null);
  }
  return { drawingInfo: h2 ? o(h2) : null, extent: $(n), geometryType: h2, fields: y, hasZ: !!b2, objectIdField: j3, timeInfo: F2 };
}
async function F(n, i3 = {}) {
  ...
  const { apiKey: o2, customParameters: s3, signal: l } = i3, { data: c } = await j(a.href, { signal: l, headers: { accept: "application/json" }, query: { ...s3, token: o2 } });
  return c;
}
async function I2(n, i3 = {}) {
...
async function T(t, n = {}) {
  const { apiKey: i3, customParameters: r, signal: a } = n, { data: o2 } = await j(t, { signal: a, headers: { accept: "application/json" }, query: { ...r, token: i3 } });
  return o2;
}
async function k(t, n = {}) {
  const i3 = "application/vnd.oai.openapi+json;version=3.0", r = G(t.links, "service-desc", i3);
  if (null == r)
    return w.warn("ogc-feature-layer:missing-openapi-page", "The OGC API-Features server does not have an OpenAPI page."), null;
  const { apiKey: a, customParameters: o2, signal: s3 } = n, { data: l } = await j(r.href, { signal: s3, headers: { accept: i3 }, query: { ...o2, token: a } });
  return l;
}
function x(e2) {
...
async function S(e2, t, n) {
  const i3 = await v(e2, t, n);
  return ct(i3);
}
async function v(n, a, c) {
  const { collection: u, layerDefinition: m, maxRecordCount: p, queryParameters: { apiKey: f2, customParameters: g2 }, spatialReference: w2, supportedCrs: b2 } = n, { links: h2 } = u, j3 = G(h2, "items", "application/geo+json") || G(h2, "http://www.opengis.net/def/rel/ogc/1.0/items", "application/geo+json");
  if (null == j3)
    throw new s2("ogc-feature-layer:missing-items-page", "Missing items url");
  const { geometry: F2, num: I3, start: T2, timeExtent: k2, where: x2 } = a;
  if (a.objectIds)
    throw new s2("ogc-feature-layer:query-by-objectids-not-supported", "Queries with objectids are not supported");
  const S2 = f.fromJSON(w2), v2 = a.outSpatialReference ?? S2, N4 = v2.isWGS84 ? null : R(v2, b2), q2 = M(F2, b2), $2 = O(k2), W = C(x2), D = I3 ?? (null != T2 && void 0 !== T2 ? 10 : p), { data: P } = await j(j3.href, { ...c, query: { ...g2, ...q2, crs: N4, datetime: $2, query: W, limit: D, startindex: T2, token: f2 }, headers: { accept: "application/geo+json" } });
  let Z2 = false;
  if (P.links) {
  ...
  const { fields: K, geometryType: A, hasZ: L, objectIdField: J } = m, U = N2(P, { geometryType: A, hasZ: L, objectIdField: J });
  if (!N4 && v2.isWebMercator) {
    for (const e2 of U)
  ...
  const z = N4 || !N4 && v2.isWebMercator ? v2.toJSON() : N, E2 = new e();
...
function q(e2) {
  if (null == e2)
    return "";
  const { xmin: t, ymin: n, xmax: i3, ymax: r } = e2;
  return `${t},${n},${i3},${r}`;
}
function O(e2) {
  if (null == e2)
...
function M(e2, t) {
  ...
  if (!n || n.isWGS84)
    return { bbox: q(e2) };
  const i3 = R(n, t);
  return null != i3 ? { bbox: q(e2), "bbox-crs": i3 } : n.isWebMercator ? { bbox: q(g(e2, f.WGS84)) } : null;
}
function $(e2) {
  var _a;
  ...
  if (!t)
    return null;
  const n = t.bbox[0], i3 = 4 === n.length, r = n[0], a = n[1], o2 = i3 ? void 0 : n[2];
  return { xmin: r, ymin: a, xmax: i3 ? n[2] : n[3], ymax: i3 ? n[3] : n[4], zmin: o2, zmax: i3 ? void 0 : n[5], spatialReference: f.WGS84.toJSON() };
}
function G(e2, t, n) {
...
//# sourceMappingURL=chunk-4PHH3CU7.js.map
